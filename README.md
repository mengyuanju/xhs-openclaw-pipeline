...
`imageCount` 只能为 3–5，`metadata` 必须是 JSON 对象。页面流程为“上传与结构预检 → 需求强度筛选 → 确认入队”；所有结构合格行完成筛选前不能提交。

如工作表已经完成业务筛选，还可提供 `是否有效`、`废弃原因`、`需求强度判定`、`判定简要说明`，页面会自动带入并允许人工修正；未提供时在页面逐条或批量判定。需求强度支持“强需 / 中需 / 弱需 / 无需”：强需、中需进入生产队列，弱需、无需只保留在筛选记录中。判定档位与理由会随准入任务传给 Worker。
