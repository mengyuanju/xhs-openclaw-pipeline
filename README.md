...
2026-09-06 已移除 OpenClaw 执行器及旧 Web 的导入、独立生成、批量生成、任务审核、统计、追踪页面和专属 API。首页统一进入 `/workbench/personal`。历史配置中的 `OPENCLAW` 在读取时归一为 `CODEX`，不会调用旧引擎；历史执行来源记录和业务数据保持原样。

## 当前功能与数据流

- `/workbench/*`：个人任务、文案审核、生图、图文审核与归档；保留重试、断点恢复、执行证据和 ZIP 下载。
- `/prompts`：提示词版本、发布与回滚。任务使用冻结的执行快照，全局修改不会污染在途任务。
- `/knowledge`：文案知识分析与发布；保留视觉知识及素材能力。
- `/settings`：生产参数、文案提供方、联网搜索、模型及并发配置。
- `/executors`、`/users`、`/profile`：执行机状态、用户管理与个人信息。

Query 进入全局文案队列，执行机按容量领取。文案经人工审核后进入生图队列；只有启用图片能力的执行机可以领取。人工重试产生新的执行代次，旧执行不能覆盖新结果。
...
默认打开 `http://127.0.0.1:3000`。登录由中心账户服务验证，初始管理员需按界面提示修改密码。
