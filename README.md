...
启用图片通道后，同一个图片容量池会按中心统一优先级领取普通生图和人工改图；中心机不执行改图模型。升级时先升级并迁移中心，再更新所有图片执行机；新版图片执行机发现中心缺少 `imageEditExecutorVersion=6` 会拒绝启动。调用图片模型的修改只由版本 6 执行机领取，以确保自动验收未通过的生成结果仍会上传显示；版本 3 执行机仅兼容确定性合成与历史恢复。
