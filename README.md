...
`POST /api/copy-generations` 可脱离完整 Worker 单独生成并保存文案。提交后会先持久化一条 `RUNNING` 生成任务，再依次执行 Query 审核、联网研究、原始版生成与首次文本质检，根据首次质检意见完整生成质检版并复检；两版内容、模型、审核证据和研究快照会作为同一条历史记录原子保存，任务同时转为 `COMPLETED`。失败任务保存安全、脱敏的错误摘要。它使用后台当前已发布的 `TEXT_SYSTEM` 提示词，不会启动图片生成或图片验收，也不会自动写入任务和文案修订表。

请求必须来自已登录管理员的同源会话，并显式确认真实模型费用。正常流程包含两次文案生成和两次文本质检；如果第一份质检版与原始版只有空白、字段顺序等非实质差异，系统会带着首次质检意见自动再生成一次。连续两次质检版都没有实际修改时返回失败且不保存记录：
...
`imageCount` 可传 `3`、`4`、`5` 或 `"auto"`，只控制两版随文案返回的 `imagePlan` 项数，不会调用图片模型。成功返回 HTTP `201`：`data.original` 是首次生成与首次质检，`data.reviewed` 是确认发生实质修改后的质检版与复检；兼容字段 `data.copy`、`data.imagePlan` 和 `data.generation.model` 均指向质检版。Query 或质检版复检拒绝时返回 HTTP `422` 且不保存半成品；联网研究失败或质检版连续未修改时返回 HTTP `502`；同一服务进程已有文案请求执行时返回 HTTP `409`。

`GET /api/copy-generations?page=1&pageSize=20` 按新到旧返回已保存的双版本历史，`pageSize` 最大为 `50`；新增的 `jobs` 字段返回最近最多 20 条仍在生成或已失败的任务。页面刷新或切换板块后会从服务端恢复这些任务，并仅在存在 `RUNNING` 任务时定时刷新；完成后任务自动离开状态区并进入历史记录。每条新记录的 `generation.timing` 包含总耗时以及选题审核、联网研究、原始版生成/质检、质检版生成/复检六阶段耗时；列表级 `statistics` 基于最近最多 1000 条有效样本返回样本数、平均耗时、P50、P95 和各阶段平均值。升级前的旧记录返回 `timing: null`，不参与统计。调用方如需进入后续生图，应通过任务文案修订接口显式保存确认后的文案。
