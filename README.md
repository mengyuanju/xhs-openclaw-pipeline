...
创建笔记时 Query 直接进入全局共享文案队列，不预先绑定执行机；尚未领取时显示“待文案执行”。任一文案执行机在当前运行数小于自身最大并发量时，都可以按队列顺序原子领取；领取成功后才记录实际文案执行机。人工审核指定文案版本后，任务进入全局生图队列。只有显式启用图片能力且当前空闲的执行机才能原子领取一条生图任务。每次执行都有独立 `executionId`；人工重试会作废旧代次，旧进程之后提交的结果会被中心服务拒绝，不能覆盖新结果。

待文案审核的任务详情在“刷新”旁提供任务级“AI生成”开关。提交审核时该选择会保存到中心任务并冻结进生图执行快照；开启时生成和验收均要求图片显示“AI生成”标识，关闭时包括人像页在内均不得显示该标识。旧任务默认开启，以保持升级前行为。
...
文案生成前的 Query 筛选由管理员「提示词 → 执行配置 → 启用 Query 筛选」控制，配置字段为 `prompt_runtime.queryReviewEnabled`，默认关闭；同页视觉规划 `visualPlanningEnabled` 默认开启。默认值用于未配置的字段，已保存的开关值保持有效。Query 筛选关闭后跳过选题审核模型调用，记录 `skipped: true`、`source: DISABLED`，继续检索和文案生成；不会关闭上述 Excel 需求强度筛选或生成后的文案审核。开关跟随执行快照保存，已冻结任务及历史重试沿用原配置。启用统一规则时仍需按页面提示发布其他必需阶段的提示词；Query 筛选关闭时不要求发布选题审核模板。

在同一页面点击「编辑 Query 筛选提示词」即可编辑 `QUERY_REVIEW_SYSTEM`。中心缺少模板时也会显示可编辑的默认候选内容，保存草稿或提交更新会创建首个版本；草稿不会自动成为执行规则。开启筛选前需发布该模板，之后的新执行使用已发布版本。
