...
export function StandaloneImageEditor({taskId,runId,copyRevisionId,asset,assets,page,runs,onChanged,onSubmitted,onBusyChange}: {
  taskId:number;runId:string;copyRevisionId:number;asset:Asset;assets?:Asset[];page:number;
  runs:Array<{id:string;result:{processing?:{type:string}}|null}>;onChanged:()=>Promise<void>;onSubmitted:()=>void;onBusyChange:(busy:boolean)=>void;
}) {
  ...
  async function act(action:()=>Promise<unknown>,pending='正在处理…',success='操作完成',submitted=false) {setBusy(true);setError('');setNotice(pending);try{await action();if(submitted)onSubmitted();await refresh();await onChanged();setNotice(success);}catch(e){setNotice('');setError(e instanceof Error?e.message:'操作失败');}finally{setBusy(false);}}
  ...
  async function submitEntity(draft=false) {
    ...
    try {
      ...
      if(!draft&&!failedPages.length)onSubmitted();
  ...
  async function submitDisclosureBatch() {
    ...
    try {
      ...
      if(!failedPages.length)onSubmitted();
  ...
  return <section className={inlineStyles.editor} aria-label="图片编辑组件" aria-busy={busy}>
      <header className={styles.header}>
        ...
        <p className={styles.description}>选择修改方式，保存后开始生图。结果可在列表中打开、预览和下载。</p>
        ...
        <div className={styles.workspace} data-mobile-view={mobileView.toLowerCase()}>
          ...
          <section className={styles.controlPanel} aria-label="图片修改控制">
            ...
            {panel==='EDIT'?<>
              ...
              <footer className={styles.actionBar}><span>{usesBillableModel?'保存后由执行机处理图片':'程序标识不调用模型'}</span><Button disabled={busy} onClick={()=>void (tab==='TEXT'&&textScope==='ALL'&&imageAssets.length>1?submitDisclosureBatch():submit())}>{busy?'正在保存…':'保存并提交生图'}</Button></footer>
            </>:<section className={styles.panelBody} aria-label="编辑记录">
              <section className={styles.restorePanel} aria-label="历史图片版本"><div><strong>历史图片版本</strong><span>恢复入口与编辑记录集中管理</span></div><Select value={history} onValueChange={setHistory} disabled={busy || !runs.some(r=>r.id!==runId)}><SelectTrigger aria-label="历史图片版本"><SelectValue placeholder="选择要恢复的图集" /></SelectTrigger><SelectContent className={styles.selectContent}>{runs.filter(r=>r.id!==runId).map(r=><SelectItem key={r.id} value={r.id}>{labels[r.result?.processing?.type??'']??'原始上传'} · {r.id.slice(0,8)}</SelectItem>)}</SelectContent></Select><Button variant="outline" disabled={!history||busy} onClick={()=>void act(()=>post(`/v1/tasks/${taskId}/image-versions/${history}/restore`,{...base(),instruction:'恢复历史图片版本'}),'正在创建恢复预览…','恢复预览请求已提交。')}>生成恢复预览</Button></section>
              ...
              {pageEdits.length?<ul className={styles.history} aria-label="图片修改记录">{pageEdits.map(e=>{
                ...
                return <li key={e.id}><div className={styles.historyTitle}><strong>{labels[e.operation]} · {alternatives.length?'待确认建议':rejectedPreview?(e.status==='ACCEPTED'?'已人工采用 · 自动验收未通过':'验收未通过 · 结果已保留'):labels[e.status]}</strong><span>第 {e.target_page} 页 · {e.created_by} · 已执行 {e.attempts??0} 次</span></div><p className={styles.historySummary}>{e.config.instruction}</p>
                  ...
                  <div className={styles.historyActions}>{e.operation==='AI_LOCAL'&&<Button variant="outline" size="sm" onClick={()=>reuseInstruction(e)}>复用说明并修改</Button>}{e.result&&<><Button size="sm" onClick={()=>{setComparisonId(e.id);setPreviewMode('COMPARE');setMobileView('PREVIEW');}}>在左侧对比</Button><a href={path(`/v1/assets/${e.result.asset_id}`)} target="_blank" rel="noreferrer">打开结果</a><a href={path(`/v1/assets/${e.result.asset_id}?download=true`)} download>下载结果</a></>}{historyActions(e).map(action=><Button key={action} size="sm" variant={action==='cancel'?'outline':undefined} className={action==='cancel'?styles.deleteAction:undefined} disabled={busy||action==='apply-suggestion'&&!selectedAlternative} onClick={()=>{setReason(action==='apply-suggestion'&&selectedAlternative?`采用「${selectedAlternative.title}」方案`:'');setHistoryCostConfirmed(false);setPendingBatchAccept(false);setPendingHistoryAction({editId:e.id,action});}}>{historyActionLabel(e,action)}</Button>)}</div>
