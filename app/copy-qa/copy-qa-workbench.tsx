...
export function CopyQaWorkbench({ role }: { role: 'ADMIN' | 'REVIEWER' }) {
  ...
  const load = useCallback(async ({ silent = false, offset = 0 } = {}) => {
    ...
    try {
      const [payload, statisticsPayload] = await Promise.all([
        apiRequest<unknown>(apiPath(`/v1/copy-qa/items?status=${encodeURIComponent(status)}&limit=${COPY_QA_LIST_LIMIT}&offset=${offset}`)),
        role === 'ADMIN' && !append
      ...
      if (currentRequestId !== listRequestId.current) return;
      setError(caught instanceof Error ? caught.message : '文案抽检队列读取失败');
    } finally {
  ...
  const visibleItems = useMemo(() => {
    ...
    return items.filter((item) => (status === 'ALL' || item.status === status)
      && (mode === 'ALL' || (mode === 'BLIND' ? item.blindReview : !item.blindReview))
      && (!keyword || `${item.anonymousCode} ${item.query ?? ''} ${item.productionBatch.anonymousCode}`.toLocaleLowerCase('zh-CN').includes(keyword)));
  ...
  async function passItem(item: CopyQaItem) {
    ...
    const approved = await confirm({
      title: '确认当前最终稿通过抽检？',
      description: `${item.anonymousCode} 将按当前内容指纹绑定的最终人工通过稿记录抽检结果。`,
      confirmLabel: '确认通过',
    });
    if (!approved) return;
    setAction('pass');
    ...
    try {
      ...
      setDetail(null);
      setMessage(`${item.anonymousCode} 已通过抽检。`);
      await load({ silent: true });
    } catch (caught) {
      setDetailError(caught instanceof Error ? caught.message : '抽检通过失败');
    } finally {
      setAction('');
    }
  }
  ...
  return <div className={styles.stack}>
    ...
    <section className="panel" aria-labelledby="copy-qa-queue-title">
      <div className={styles.toolbar}>
        <div><div><h2 id="copy-qa-queue-title">待抽检队列</h2><p className="subtle">默认单条处理错误；整批打回是单独的高风险管理操作。</p></div></div>
        <div>
      ...
      <div className={styles.scopeNote}>结果状态由服务端直接筛选；Query、匿名编号和评审模式搜索仅作用于当前已加载的 {items.length} 条。{hasMore ? '仍有更多结果可继续加载。' : '当前状态结果已全部加载。'}</div>
      ...
      {loading ? <div className="empty-state" role="status"><LoaderCircle className="animate-spin" size={20} />正在读取抽检队列…</div>
        : visibleItems.length === 0 ? <div className="empty-state">{items.length ? '没有符合当前筛选条件的抽检项。' : '当前没有待处理的文案抽检项。'}</div>
    ...
    <Dialog open={detail !== null || detailLoading} onOpenChange={(open) => { if (!open && !action) closeItemDetail(); }}>
      <DialogContent className={styles.dialog}>
        <div className={styles.dialogHeader}><div><DialogTitle>{detail?.anonymousCode ?? '读取抽检详情'}</DialogTitle><DialogDescription>{detail ? '核对内容指纹绑定的最终人工通过稿' : '正在读取抽检详情…'}</DialogDescription></div>{detail && <span className="pill">{detail.blindReview ? <><EyeOff size={13} />独立盲评</> : <><Eye size={13} />非盲评</>}</span>}</div>
        ...
        {detail && copy && <>
          ...
          <article className={styles.copy} aria-label="最终人工通过文案"><span className="pill">最终人工通过稿</span>{copy.title && <h3>{copy.title}</h3>}<p className={styles.copyBody}>{copy.body || '最终稿正文为空'}</p>{copy.tags.length > 0 && <div className={styles.tags}>{copy.tags.map((tag) => <span className="pill" key={tag}>#{tag}</span>)}</div>}</article>
          {!detail.blindReview && <section className={styles.source} aria-labelledby="copy-qa-source-title"><h3 id="copy-qa-source-title">管理员来源信息</h3><div className={styles.metadata}><div><small>正式任务</small><strong>{detail.taskId ? `#${detail.taskId}` : '未记录'}</strong></div><div><small>最终审批账号</small><strong>{detail.finalApproverAccountId ? `账号 #${detail.finalApproverAccountId}` : '未记录'}</strong></div><div><small>生产批次</small><strong>{detail.productionBatchId ? `#${detail.productionBatchId}` : detail.productionBatch.anonymousCode}</strong></div></div></section>}
          {detailError && <div className="notice error" role="alert">{detailError}</div>}
          <div className={styles.footer}><span className="subtle">抽检结论只绑定当前展示的最终修订版。</span><div><DialogClose asChild><Button unstyled className="button" type="button" disabled={Boolean(action)}>关闭</Button></DialogClose>{detail.status === 'PENDING' && detail.capabilities.canReturnSingle && <Button unstyled className="button danger" type="button" disabled={Boolean(action)} onClick={() => beginSingleReturn(detail)}>仅打回此条</Button>}{canReleaseCopyQaFreezeRest(detail) && <Button unstyled className="button primary" type="button" disabled={Boolean(action)} onClick={() => beginReleaseRest(detail)}>放行同批其余</Button>}{detail.status === 'RETURNED' && canStartCopyQaBatchReturn(detail) && <Button unstyled className="button danger" type="button" disabled={Boolean(action)} onClick={() => { void beginBatchReturn(detail); }}>升级整批打回</Button>}{detail.status === 'PENDING' && detail.capabilities.canPass && <Button unstyled className="button primary" type="button" disabled={Boolean(action)} onClick={() => { void passItem(detail); }}>{action === 'pass' ? '提交中…' : '通过最终稿'}</Button>}</div></div>
        </>}
      </DialogContent>
    </Dialog>
    ...
    <Dialog open={returnItem !== null} onOpenChange={(open) => { if (!open && action !== 'return-single') { setReturnItem(null); setReturnCompleted(false); } }}>
      <DialogContent className={styles.dialog}>
        <div className={styles.dialogHeader}><div><DialogTitle>{returnCompleted ? '当前错误项已单条打回' : '只打回当前错误项'}</DialogTitle><DialogDescription>{returnItem?.sampleKind === 'MANDATORY_RECHECK'
          ? returnCompleted
            ? '强制复检错误项已经退回修改；完成真实修改后会再次进入强制复检。'
            : `${returnItem.anonymousCode} 将退回修改；强制复检不会放行同批其余或扩大为整批打回。`
          : returnCompleted
        ...
        {!returnCompleted ? <>
          ...
          <div className={styles.footer}><span className="subtle">默认最小影响范围：当前单条任务。</span><div><DialogClose asChild><Button unstyled className="button" type="button" disabled={action === 'return-single'}>取消</Button></DialogClose><Button unstyled className="button danger" type="button" disabled={action === 'return-single'} onClick={() => { void submitSingleReturn(); }}>{action === 'return-single' ? '打回中…' : '确认仅打回此条'}</Button></div></div>
        </> : returnItem && <>
          <div className={styles.impact}><strong>错误项：{returnItem.anonymousCode}</strong><span>单条打回已生效；关闭弹窗也不会自动放行或扩大范围。</span></div>
          {returnItem.sampleKind === 'MANDATORY_RECHECK'
            ? <>
              <div className="notice">强制复检只处理当前返工项；完成真实文案修改后会再次进入强制复检。</div>
              <div className={styles.footer}><span className="subtle">此处不会提供整批打回或放行同批其余操作。</span><DialogClose asChild><Button unstyled className="button" type="button">完成</Button></DialogClose></div>
            </>
            : <>
    ...
    <Dialog open={batchOpen} onOpenChange={(open) => { if (!open && action !== 'return-batch') closeBatchReturn(); }}>
      <DialogContent className={styles.dialog}>
        ...
        <div className="field"><label htmlFor="copy-qa-batch-note">整批打回说明（必填）</label><Textarea id="copy-qa-batch-note" value={batchNote} maxLength={500} rows={5} disabled={action === 'return-batch'} placeholder="说明为何需要扩大到整批，以及统一返修要求" onChange={(event) => { setBatchNote(event.target.value); setBatchError(''); }} /></div>
