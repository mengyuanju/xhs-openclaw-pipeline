          ...
          <tbody>{nodes.map((node) => {
            ...
            return <tr key={node.id}>
              ...
              <td data-label="生图任务">{node.imageWorkerEnabled
                ? <div className="executor-capacity"><strong>{node.imageRunningCount} / {node.imageConcurrency} 执行中</strong><span>空闲 {imageAvailable} 个槽位</span><span>{node.imageEditExecutorVersion >= 11?'支持批量、多产品与框内全部同款替换':node.imageEditExecutorVersion >= 10?'支持批量与多产品替换，需升级全部同款替换':node.imageEditExecutorVersion >= 8?'支持 AI 改图、程序标识与失败图定向修复':node.imageEditExecutorVersion >= 7?'支持 AI 改图，需升级程序标识':node.imageEditExecutorVersion >= 3?'仅支持合成与恢复，需升级 AI 改图':'需更新执行机才能改图'}</span></div>
