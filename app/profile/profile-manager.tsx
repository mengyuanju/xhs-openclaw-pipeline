...
function PasswordForm({
  ...
  return <form className="profile-form password-change-form" onSubmit={onSubmit} aria-busy={isBusy}>
    ...
    <div className="profile-password-grid">
      ...
      <div className="field"><label htmlFor={`${idPrefix}-confirm-password`}>确认新密码</label><Input className="input" id={`${idPrefix}-confirm-password`} name="confirmPassword" type="password" autoComplete="new-password" minLength={6} required disabled={isBusy} /></div>
    </div>
    <div className="profile-form-actions">
      <span>请勿与其他平台共用同一密码</span>
      <div className="password-change-actions">
        {onSwitchAccount && <Button unstyled className="button" type="button" onClick={onSwitchAccount} disabled={isBusy}>退出并切换账号</Button>}
        <Button unstyled className="button primary" type="submit" disabled={isBusy}>{isSubmitting ? '修改中…' : submitLabel}</Button>
      </div>
    </div>
  </form>;
}
...
export function ProfileManager({ user }: { user: ProfileUser }) {
  ...
  async function switchAccount() {
    ...
    try {
      ...
      // `reauth=1` keeps account switching available even if cookie cleanup fails.
    } finally {
  ...
  if (user.mustChangePassword) return <Dialog open onOpenChange={() => undefined}>
    <DialogContent
      className="forced-password-dialog"
    ...
    >
      <div className="forced-password-header">
        <span className="forced-password-icon" aria-hidden="true"><LockKeyhole size={23} /></span>
        <div>
          <span className="section-kicker">首次登录安全设置</span>
          <DialogTitle className="forced-password-title">必须先修改初始密码</DialogTitle>
          <DialogDescription className="forced-password-description">
            当前账号仍在使用系统初始密码。为保护后台数据，修改密码并重新登录前，其他功能暂不可用。
          </DialogDescription>
        </div>
      </div>
      {error && <div className="notice error forced-password-error" role="alert">{error}</div>}
      <PasswordForm
        autoFocusCurrent
        idPrefix="forced"
        isBusy={Boolean(busy)}
        ...
        submitLabel="修改密码并重新登录"
      />
      <p className="forced-password-footnote"><ShieldCheck size={15} />修改成功后系统会退出当前会话，请使用新密码重新登录。</p>
    </DialogContent>
  </Dialog>;
