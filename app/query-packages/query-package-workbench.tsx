...
const PRODUCIBLE_PACKAGE_STATUSES = ['READY', 'PARTIALLY_USED'];
const PACKAGE_STATUS_LABELS: Record<string, string> = {
  DRAFT: '待筛选',
  IMPORTED: '待筛选',
  SCREENING: '筛选中',
  READY: '可投产',
  PARTIALLY_USED: '部分已投产',
  USED_UP: '已全部投产',
  ABANDONED: '已停用',
  PRODUCED: '已投产',
  CLOSED: '已完成',
...
function listFromPayload(value: unknown) {
  return normalizePackagePage(value);
}

type AssignableUser = { id: number; username: string; displayName: string };

function userListFromPayload(value: unknown): AssignableUser[] {
  const row = value && typeof value === 'object' ? value as Record<string, unknown> : null;
  const entries = Array.isArray(value) ? value : Array.isArray(row?.items) ? row.items : [];
  return entries.flatMap((entry) => {
    if (!entry || typeof entry !== 'object') return [];
    const user = entry as Record<string, unknown>;
    const id = Number(user.id);
    if (!Number.isSafeInteger(id) || id < 1 || typeof user.username !== 'string'
      || user.role !== 'USER' || (user.status && user.status !== 'ACTIVE')) return [];
    return [{ id, username: user.username, displayName: typeof user.displayName === 'string' && user.displayName.trim() ? user.displayName : user.username }];
  });
}

export function QueryPackageWorkbench({ role }: { role: 'ADMIN' | 'USER' }) {
  const confirm = useConfirmDialog();
  const [packages, setPackages] = useState<QueryPackageSummary[]>([]);
  const [settings, setSettings] = useState<WorkflowQualitySettings | null>(null);
  ...
  const [itemStatus, setItemStatus] = useState<QueryPackageDecision | QueryPackageValidationStatus | 'ALL'>('ALL');
  const [itemPage, setItemPage] = useState(1);
  const [checkedItemIds, setCheckedItemIds] = useState<number[]>([]);
  const [productionItemIds, setProductionItemIds] = useState<number[]>([]);
  ...
  const canImport = role === 'ADMIN' || settings?.queryPackage.workerImportEnabled === true;
  ...
  const load = useCallback(async ({ silent = false, offset = 0 } = {}) => {
    ...
    try {
      const [packagePayload, settingsPayload] = await Promise.all([
        apiRequest<unknown>(apiPath(`/v1/query-packages?limit=${QUERY_PACKAGE_LIST_LIMIT}&offset=${offset}`)),
        !append ? apiRequest<unknown>(apiPath('/v1/workflow-quality-settings')).catch(() => null) : Promise.resolve(null),
      ]);
      if (currentRequestId !== packageListRequestId.current) return;
      const page = listFromPayload(packagePayload);
      ...
      if (!append) setSettings(normalizeWorkflowQualitySettings(settingsPayload));
      setError('');
    } catch (caught) {
      if (currentRequestId !== packageListRequestId.current) return;
  ...
  const visibleItems = useMemo(() => {
    ...
    return detail.items.filter((item) => (itemStatus === 'ALL'
      || (item.validationStatus === 'READY' ? item.screeningDecision === itemStatus : item.validationStatus === itemStatus))
  ...
  const productionItemIdSet = useMemo(() => new Set(productionItemIds), [productionItemIds]);
  ...
  async function openPackage(id: number) {
    ...
    try {
      ...
      const next = normalizePackageDetail(payload);
      if (!next) throw new Error('中心返回的词包详情不完整');
      setDetail(next);
      setProductionItemIds(next.items
        .filter((item) => item.validationStatus === 'READY' && item.screeningDecision === 'SELECTED' && !item.taskId)
        .map((item) => item.id)
        .slice(0, QUERY_PACKAGE_PRODUCTION_LIMIT));
    } catch (caught) {
      if (!canCommitLatestRequest(packageDetailRequestId.current, currentRequestId, controller.signal.aborted)) return;
  ...
  async function createPackage(event: FormEvent<HTMLFormElement>) {
    ...
    if (!canImport || creating || readingImportFile || parsedImport.error) return;
    setCreating(true);
    setImportError('');
    setMessage('');
    try {
  ...
  async function screen(decision: 'SELECT' | 'REJECT') {
    ...
    setDetailError('');
    try {
      await apiRequest(apiPath(`/v1/query-packages/${detail.id}/screening`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      setMessage(decision === 'SELECT'
        ? `已将 ${checkedItemIds.length} 条 Query 标记为通过。`
        : `已淘汰 ${checkedItemIds.length} 条 Query。`);
      await refreshDetail();
    } catch (caught) {
  ...
  async function createProductionBatch() {
    if (!detail || !PRODUCIBLE_PACKAGE_STATUSES.includes(detail.status) || productionItemIds.length === 0 || acting) return;
    if (productionItemIds.length > QUERY_PACKAGE_PRODUCTION_LIMIT) {
      setDetailError(`单个生产批次最多包含 ${QUERY_PACKAGE_PRODUCTION_LIMIT} 条 Query，请拆分后再创建。`);
      return;
    }
    const approved = await confirm({
      title: `将所选 ${productionItemIds.length} 条通过项创建为一个生产批次？`,
      description: `词包“${detail.name}”本次只投产已勾选且尚未投产的 ${productionItemIds.length} 条。剩余通过项可在之后拆成其他生产批次。`,
      confirmLabel: `创建 ${productionItemIds.length} 条作业`,
    });
    if (!approved) return;
    setActing('produce');
    setDetailError('');
    try {
      await apiRequest(apiPath(`/v1/query-packages/${detail.id}/production-batches`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ expectedVersion: detail.version, itemIds: productionItemIds, requestId: crypto.randomUUID() }),
      });
      setMessage(`所选 ${productionItemIds.length} 条通过项已创建为独立生产批次，并进入文案生产流程。`);
      await refreshDetail();
    } catch (caught) {
      setDetailError(caught instanceof Error ? caught.message : '正式作业创建失败');
    } finally {
      setActing('');
    }
  }
  ...
  async function abandon() {
    ...
    try {
      await apiRequest(apiPath(`/v1/query-packages/${abandonPackage.id}/abandon`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ expectedVersion: abandonPackage.version, reason: abandonReason.trim(), requestId: crypto.randomUUID() }),
      });
  ...
  async function openAssignment(item: QueryPackageSummary) {
    ...
    try {
      ...
      const users = userListFromPayload(payload);
      setAssignableUsers(users);
      const current = users.find((user) => user.username === item.assignedToUserId);
      setAssignedUserId(current ? String(current.id) : '');
    } catch (caught) {
      if (!canCommitLatestRequest(assignmentRequestId.current, currentRequestId, controller.signal.aborted)) return;
      setAssignError(caught instanceof Error ? caught.message : '作业人员列表读取失败');
    } finally {
  ...
  async function saveAssignment() {
    if (!assignPackage || !assignedUserId || acting) return;
    const user = assignableUsers.find((candidate) => String(candidate.id) === assignedUserId);
    if (!user) return;
    setActing('assign');
    setAssignError('');
    try {
      await apiRequest(apiPath(`/v1/query-packages/${assignPackage.id}/assignee`), {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ expectedVersion: assignPackage.version, assignedToUserId: user.username, assignedToAccountId: user.id }),
      });
      setMessage(`词包“${assignPackage.name}”已分配给 ${user.displayName}。`);
      closeAssignmentDialog();
      await load({ silent: true });
    } catch (caught) {
      setAssignError(caught instanceof Error ? caught.message : '词包分配失败');
    } finally {
      setActing('');
    }
  }
  ...
  const productionCandidates = pagedItems.filter((item) => item.validationStatus === 'READY' && item.screeningDecision === 'SELECTED' && !item.taskId);
  const allAvailableProductionItems = detail?.items.filter((item) => item.validationStatus === 'READY'
  ...
  const allProductionChecked = productionCandidates.length > 0 && productionCandidates.every((item) => productionItemIdSet.has(item.id));
  ...
  return <div className={styles.stack}>
    ...
    <section className="panel" aria-labelledby="query-package-list-title">
      <div className={styles.toolbar}>
        <div>
          ...
          <p className="subtle">作业人员始终可以筛选分配给自己的词包并将通过项投产；导入权限由管理员单独控制。</p>
        </div>
        <div className={styles.toolbarGroup}>
          ...
          {canImport && <Button unstyled className="button primary" type="button" onClick={() => { setImportError(''); setImportOpen(true); }}>
            <Upload aria-hidden="true" size={15} />导入 Query 词包
          </Button>}
        </div>
      </div>
      <div className={styles.scopeNote}>名称、负责人和状态筛选当前覆盖已加载的 {packages.length} 个词包。{hasMorePackages ? '仍有更多词包，可继续加载后再筛选。' : '词包列表已全部加载。'}</div>

      <div className={styles.toolbar}>
        <div className={styles.toolbarGroup}>
          <SearchInput className={styles.search} value={search} onValueChange={setSearch} placeholder="搜索词包名称或负责人" />
          <label>状态
        ...
        {!canImport && <span className="pill">当前账号不可新建词包</span>}
      </div>
      ...
      {loading
        ...
        : visiblePackages.length === 0
          ...
          : <div className={`table-wrap mobile-cards ${styles.table}`}><table>
            <thead><tr><th>词包</th><th>筛选进度</th><th>状态 / 负责人</th><th>创建时间</th><th>操作</th></tr></thead>
            <tbody>{visiblePackages.map((item) => {
              ...
              return <tr key={item.id}>
                ...
                <td data-label="筛选进度"><div className={styles.counts}>
                  ...
                  <div className={styles.countLine}><span>待筛 {item.counts.pending}</span><span>通过 {item.counts.selected}</span><span>淘汰 {item.counts.rejected}</span><span>投产 {item.counts.produced}</span></div>
                </div></td>
                <td data-label="状态 / 负责人"><div className={styles.nameCell}><span className="pill">{PACKAGE_STATUS_LABELS[item.status] ?? item.status}</span><small>{item.assignedToDisplayName || item.assignedToUserId || '尚未分配'}</small></div></td>
                ...
                <td className="row-action" data-label="操作"><div className={styles.actions}>
                  ...
                  {role === 'ADMIN' && <Button unstyled className="button small" type="button" onClick={() => { void openAssignment(item); }}><UserRoundCog size={14} />分配</Button>}
                  {role === 'ADMIN' && ['USED_UP', 'ABANDONED'].includes(item.status)
                    ...
                    : role === 'ADMIN' && <Button unstyled className="button small danger" type="button" onClick={() => prepareAbandon(item)}><XCircle size={14} />废弃词包</Button>}
                </div></td>
              </tr>;
            })}</tbody>
          </table></div>}
    ...
    <Dialog open={importOpen} onOpenChange={(open) => { if (creating) return; if (open) setImportOpen(true); else closeImportDialog(); }}>
      <DialogContent className={styles.dialog}>
        <div className={styles.dialogHeader}><DialogTitle>导入 Query 词包</DialogTitle><DialogDescription>每行一条 Query，最多 5000 条；导入后只进入人工筛选，不会直接创建正式作业。</DialogDescription></div>
    ...
    <Dialog open={detail !== null || detailLoading} onOpenChange={(open) => { if (!open && !acting) closePackageDetail(); }}>
      <DialogContent className={styles.screeningDialog}>
        <div className={styles.screeningHead}><div><DialogTitle>{detail?.name ?? '读取词包'}</DialogTitle><DialogDescription>{detail ? `词包 #${detail.id} · 逐条筛选后，将通过项创建为正式作业。` : '正在读取词包详情…'}</DialogDescription></div>{detail && <span className="pill">{PACKAGE_STATUS_LABELS[detail.status] ?? detail.status}</span>}</div>
        {detail && <div className={styles.screeningStats}><span className="pill">全部 {detail.counts.total}</span><span className="pill">待筛 {detail.counts.pending}</span><span className="pill">通过 {detail.counts.selected}</span><span className="pill">淘汰 {detail.counts.rejected}</span><span className="pill">已投产 {detail.counts.produced}</span></div>}
        {detail && <div className={styles.screeningToolbar}><div className={styles.toolbarGroup}><SearchInput className={styles.search} value={itemSearch} onValueChange={setItemSearch} placeholder="搜索 Query 或外部编号" /><Select value={itemStatus} onValueChange={(value) => setItemStatus(value as typeof itemStatus)}><SelectTrigger className={styles.filterSelect}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ALL">全部筛选结果</SelectItem><SelectItem value="PENDING">待筛选</SelectItem><SelectItem value="SELECTED">已通过</SelectItem><SelectItem value="REJECTED">已淘汰</SelectItem><SelectItem value="INVALID">内容无效</SelectItem><SelectItem value="DUPLICATE">重复项</SelectItem><SelectItem value="TASK_CREATED">已创建作业</SelectItem></SelectContent></Select></div><div className={styles.toolbarGroup}><Input value={screeningReason} maxLength={300} placeholder="批量淘汰时填写原因" aria-label="筛选原因" onChange={(event) => setScreeningReason(event.target.value)} /><Button unstyled className="button small primary" type="button" disabled={!checkedItemIds.length || Boolean(acting)} onClick={() => { void screen('SELECT'); }}><CheckCircle2 size={14} />通过 {checkedItemIds.length || ''}</Button><Button unstyled className="button small danger" type="button" disabled={!checkedItemIds.length || Boolean(acting)} onClick={() => { void screen('REJECT'); }}><XCircle size={14} />淘汰 {checkedItemIds.length || ''}</Button><Button unstyled className="button small" type="button" disabled={!PRODUCIBLE_PACKAGE_STATUSES.includes(detail.status) || !allAvailableProductionItems.length || Boolean(acting)} onClick={() => setProductionItemIds(allAvailableProductionItems.slice(0, QUERY_PACKAGE_PRODUCTION_LIMIT).map((item) => item.id))}>选择全部待投产（{allAvailableProductionItems.length}）</Button><Button unstyled className="button small" type="button" disabled={!productionItemIds.length || Boolean(acting)} onClick={() => setProductionItemIds([])}>清空投产选择</Button></div></div>}
          ...
          {detailLoading && !detail ? <div className="empty-state"><LoaderCircle className="animate-spin" size={20} />正在读取词包详情…</div>
            : detailError && !detail ? <div className="notice error" role="alert">{detailError}</div>
              : detail && visibleItems.length ? <><div className="table-wrap"><table>
                <thead><tr><th><Checkbox aria-label="选择本页可筛选 Query" checked={allVisibleChecked} onChange={(event) => setCheckedItemIds((current) => updateQueryItemSelection(current, screenableItems.map((item) => item.id), event.target.checked))} /></th><th>序号</th><th>Query</th><th>筛选结果</th><th><Checkbox aria-label="选择本页待投产通过项" checked={allProductionChecked} onChange={(event) => setProductionItemIds((current) => updateQueryItemSelection(current, productionCandidates.map((item) => item.id), event.target.checked))} /> 本次投产</th><th>正式作业</th></tr></thead>
                <tbody>{pagedItems.map((item) => {
                  const screenable = item.validationStatus === 'READY' && !item.taskId;
                  const canProduceItem = item.validationStatus === 'READY' && item.screeningDecision === 'SELECTED' && !item.taskId;
                  return <tr key={item.id}><td data-label="筛选选择"><Checkbox aria-label={`选择第 ${item.rowNumber} 条 Query 进行筛选`} checked={checkedItemIdSet.has(item.id)} disabled={!screenable} onChange={(event) => setCheckedItemIds((current) => updateQueryItemSelection(current, [item.id], event.target.checked))} /></td><td data-label="序号">{item.rowNumber}</td><td className={styles.queryCell} data-label="Query"><strong>{item.query}</strong>{item.externalId && <div className={styles.reason}>外部编号：{item.externalId}</div>}{item.screeningReason && <div className={styles.reason}>筛选说明：{item.screeningReason}</div>}</td><td data-label="筛选结果"><span className="pill">{item.validationStatus === 'READY' ? DECISION_LABELS[item.screeningDecision] : VALIDATION_STATUS_LABELS[item.validationStatus]}</span></td><td data-label="本次投产"><Checkbox aria-label={`将第 ${item.rowNumber} 条 Query 加入本次生产批次`} checked={productionItemIdSet.has(item.id)} disabled={!canProduceItem || !PRODUCIBLE_PACKAGE_STATUSES.includes(detail.status)} onChange={(event) => setProductionItemIds((current) => updateQueryItemSelection(current, [item.id], event.target.checked))} /></td><td data-label="正式作业">{item.taskId ? `#${item.taskId}` : '—'}</td></tr>;
                })}</tbody>
        ...
        <div className={styles.screeningFooter}>{detailError ? <span className="notice error" role="alert">{detailError}</span> : <span className="subtle">筛选已选 {checkedItemIds.length} 条 · 本次生产批次 {productionItemIds.length} 条{detail && !PRODUCIBLE_PACKAGE_STATUSES.includes(detail.status) ? ' · 完成全部待筛项后方可投产' : ''}</span>}<div><Button unstyled className="button" type="button" disabled={Boolean(acting)} onClick={closePackageDetail}>关闭</Button>{detail && role === 'ADMIN' && ['USED_UP', 'ABANDONED'].includes(detail.status) ? <Button unstyled className="button danger" type="button" disabled={Boolean(acting)} onClick={() => { void preparePermanentDelete(detail); }}><Trash2 size={14} />永久删除词包</Button> : detail && role === 'ADMIN' && <Button unstyled className="button danger" type="button" disabled={Boolean(acting)} onClick={() => prepareAbandon(detail)}><XCircle size={14} />废弃词包</Button>}<Button unstyled className="button primary" type="button" disabled={!detail || !PRODUCIBLE_PACKAGE_STATUSES.includes(detail.status) || productionItemIds.length === 0 || Boolean(acting)} onClick={() => { void createProductionBatch(); }}><PackageCheck size={15} />创建生产批次（{productionItemIds.length}）</Button></div></div>
      </DialogContent>
    </Dialog>

    <Dialog open={abandonPackage !== null} onOpenChange={(open) => { if (!open && acting !== 'abandon') setAbandonPackage(null); }}>
      <DialogContent className={styles.dialog}>
        <div className={styles.dialogHeader}><DialogTitle>废弃 Query 词包</DialogTitle><DialogDescription>停止继续筛选和投产这个词包。已经创建的正式作业及其审核、图片和交付数据全部保留。</DialogDescription></div>
        {abandonPackage && <form className={styles.deleteForm} onSubmit={(event) => { event.preventDefault(); void abandon(); }}>
          <div className={styles.fileRow}><strong>{abandonPackage.name}</strong><small>候选 {abandonPackage.counts.total} 条 · 已投产 {abandonPackage.counts.produced} 条</small></div>
    ...
    <Dialog open={assignPackage !== null} onOpenChange={(open) => { if (!open && acting !== 'assign') closeAssignmentDialog(); }}>
      <DialogContent className={styles.dialog}>
        <div className={styles.dialogHeader}><DialogTitle>分配 Query 词包</DialogTitle><DialogDescription>分配后，作业人员可以继续筛选该词包，并把通过项拆分为一个或多个生产批次。</DialogDescription></div>
        {assignPackage && <div className={styles.importForm}>
          <div className={styles.fileRow}><strong>{assignPackage.name}</strong><small>{assignPackage.counts.pending} 条待筛 · {assignPackage.counts.selected} 条已通过</small></div>
          {assignLoading ? <div className="empty-state"><LoaderCircle className="animate-spin" size={18} />正在读取作业人员…</div> : assignableUsers.length ? <div className="field"><label htmlFor="query-package-assignee">作业人员</label><Select value={assignedUserId} onValueChange={(value) => { setAssignedUserId(value); setAssignError(''); }}><SelectTrigger id="query-package-assignee"><SelectValue placeholder="选择作业人员" /></SelectTrigger><SelectContent>{assignableUsers.map((user) => <SelectItem key={user.id} value={String(user.id)}>{user.displayName} · {user.username}</SelectItem>)}</SelectContent></Select></div> : <div className="empty-state">没有可分配的启用中作业人员。</div>}
          {assignError && <div className="notice error" role="alert">{assignError}</div>}
          <div className={styles.dialogFooter}><span>重新分配会更新词包负责人，不改变已经创建的正式作业。</span><div className={styles.dialogButtons}><DialogClose asChild><Button unstyled className="button" type="button" disabled={acting === 'assign'}>取消</Button></DialogClose><Button unstyled className="button primary" type="button" disabled={!assignedUserId || assignLoading || acting === 'assign'} onClick={() => { void saveAssignment(); }}>{acting === 'assign' ? '分配中…' : '确认分配'}</Button></div></div>
        </div>}
      </DialogContent>
    </Dialog>
