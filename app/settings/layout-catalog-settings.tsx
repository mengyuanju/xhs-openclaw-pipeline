...
export function LayoutCatalogSettings({ remote = false }: { remote?: boolean }) {
  ...
  async function load() {
    setError('');
    try { setRecord(await apiRequest<CatalogRecord>(endpoint)); }
  ...
  const rows = templates.filter(item => (family === 'all' || item.layoutKind === family) && `${item.name} ${item.layoutTemplate} ${item.description} ${item.suitableContent}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return <section className="panel settings-section layout-catalog-section" aria-labelledby="layout-catalog-title" aria-busy={busy}>
    ...
    {record && <>
      <div className="form-grid">
        <label className="field">搜索模板<Input placeholder="编码、含义或适合内容" value={search} onChange={event => setSearch(event.target.value)} /></label>
        <div className="field"><label htmlFor="layout-family">版式分类</label><Select value={family} onValueChange={setFamily}><SelectTrigger id="layout-family"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">所有分类</SelectItem>{Object.entries(LAYOUT_FAMILIES).map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}</SelectContent></Select></div>
      ...
      {templates.length === 0 ? <p className="notice">目录中还没有模板，可导入内置的 10 类、27 个模板。</p> : <div className="layout-catalog-table" tabIndex={0} role="region" aria-label="布局模板表格，可横向滚动">
        <table><thead><tr><th>分类 / 中文类型</th><th>模板编码</th><th>具体排版含义</th><th>适合内容</th><th>状态与操作</th></tr></thead><tbody>{rows.map(item => <tr key={`${item.layoutTemplate}-${item.templateVersion}`}>
          ...
          <td><label className="switch-field"><Switch aria-label={`启用 ${item.layoutTemplate} 版本 ${item.templateVersion}`} disabled={busy} checked={item.enabled} onChange={event => void mutate({ operation: 'REPLACE', catalog: { ...catalog, templates: templates.map(candidate => candidate === item ? { ...candidate, enabled: event.target.checked } : candidate) } }, '启用状态已保存')} /><span>{item.enabled ? '已启用' : '未启用'}</span></label><Button variant="ghost" size="sm" disabled={busy} onClick={() => setEditing(item)}>编辑新版本</Button></td>
        </tr>)}</tbody></table>{rows.length === 0 && <p className="subtle">没有匹配的模板。</p>}
      </div>}
