...
export function ReviewPanel({ task }: { task: any }) {
  ...
  return <div className="review-grid">
    <section className="stack">
      <form className="panel" onSubmit={saveText}>
        ...
        <div className="form-grid">
          ...
          <div className="field full inline"><button className="button primary" disabled={busy}>保存新版本</button><span className="subtle">不会覆盖上一版。</span></div>
        </div>
      </form>

      <section className="panel">
        <div className="panel-head"><h2>图片审核与修改</h2><span className="subtle">共 {task.assets.length} 个版本</span></div>
        <form className="inline" onSubmit={uploadImage} style={{marginBottom: 16}}><input className="input" style={{flex: 1}} name="file" type="file" accept="image/png,image/jpeg,image/webp" required /><button className="button" disabled={busy}>上传参考图</button></form>
        <div className="field" style={{marginBottom: 16}}><label htmlFor="image-instruction">AI 图片修改要求（应用到你选择的图片）</label><input className="input" id="image-instruction" value={imageInstruction} onChange={(event) => setImageInstruction(event.target.value)} maxLength={1_000} placeholder="如：保留桌面主体，移除背景杂物，保持自然光" /></div>
        {task.assets.length === 0 ? <div className="empty-state">还没有图片。生成任务完成后会出现在这里，也可以先上传参考图。</div> : <div className="asset-grid">{task.assets.map((asset: any) => <article className="asset-card" key={asset.id}><img src={`/api/assets/${asset.id}`} alt={`${asset.kind} 图片版本 ${asset.revision}`} /><div className="asset-actions"><StatusPill value={asset.kind === 'REFERENCE' ? 'DRAFT' : 'COMPLETED'} /><button className="button small" disabled={busy} onClick={() => editImage(asset.id, { type: 'rotate', degrees: 90 }, '旋转')}>旋转 90°</button><button className="button small" disabled={busy} onClick={() => editImage(asset.id, { type: 'crop-3x4' }, '3:4 裁切')}>裁成 3:4</button><button className="button small" disabled={busy || !imageInstruction.trim()} onClick={() => editImage(asset.id, { type: 'ai-edit', instruction: imageInstruction }, 'AI 编辑请求入队')}>AI 图生图</button></div></article>)}</div>}
        {task.imageEditRequests.length > 0 && <div className="history" style={{marginTop: 16}}>{task.imageEditRequests.slice().reverse().map((request: any) => <div className="history-item" key={`edit-${request.id}`}><strong>图片编辑 #{request.id}</strong> · {request.status}<div className="subtle">{request.instruction}{request.error ? ` · ${request.error}` : ''}</div></div>)}</div>}
      </section>
    </section>
    ...
    <aside className="stack">
      <section className="panel">
        ...
        <div className="field"><label htmlFor="review-note">审核备注 / 驳回原因</label><textarea className="textarea" id="review-note" style={{minHeight: 110}} value={note} onChange={(event) => setNote(event.target.value)} maxLength={2_000} /></div>
        <div className="inline" style={{marginTop: 12}}><button className="button primary" disabled={busy || approvalBlockedByQc} onClick={() => review('APPROVED')}>通过</button><button className="button danger" disabled={busy} onClick={() => review('REJECTED')}>驳回</button>{task.config?.reviewStatus === 'APPROVED' && <button className="button" disabled={busy} onClick={() => review('WAITING_REVIEW')}>重新打开</button>}</div>
        <p className="subtle" style={{marginTop: 12}}>{approvalBlockedByQc ? `当前质检状态为 ${latestRun.qcDisposition}，不能标记为可交付；请完成真实生成。` : '通过条件：存在当前文案版本，并且至少有一张生成或修订后的交付图片。'}</p>
      </section>
      {message && <div className={message.includes('失败') || message.includes('必须') || message.includes('不存在') ? 'notice error' : 'notice success'}>{message}</div>}
      ...
      <section className="panel"><div className="panel-head"><h3>固定生产配置</h3></div><div className="history"><div className="history-item">文本提示词 <span className="mono">{task.config?.textPromptSha256?.slice(0, 12)}…</span></div><div className="history-item">图片提示词 <span className="mono">{task.config?.imagePromptSha256?.slice(0, 12)}…</span></div><div className="history-item">目标图片数 {task.config?.imageCount || 3}</div></div></section>
    </aside>
  </div>;
}
