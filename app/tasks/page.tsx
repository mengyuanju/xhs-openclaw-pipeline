...
export default async function TasksPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    ...
    <form className="panel form-grid" method="get" style={{marginBottom: 18}}>
      ...
      <div className="field full inline"><button className="button primary">筛选</button><Link className="button" href="/tasks">清空</Link><span className="subtle">共 {result.pagination.totalItems} 条</span></div>
    </form>
    {result.data.length === 0 ? <div className="panel empty-state">没有符合当前条件的任务。</div> : <><div className="table-wrap"><table><thead><tr><th>ID</th><th>选题</th><th>外部 ID</th><th>图片数</th><th>生成状态</th><th>审核状态</th><th></th></tr></thead><tbody>{result.data.map((task: any) => <tr key={task.id}><td className="mono">#{task.id}</td><td className="query-cell">{task.query}</td><td className="mono">{task.config?.externalId || '—'}</td><td>{task.config?.imageCount || 3}</td><td><StatusPill value={task.status} /></td><td><StatusPill value={task.config?.reviewStatus} /></td><td><Link className="button small" href={`/tasks/${task.id}`}>打开审核</Link></td></tr>)}</tbody></table></div><nav className="pagination" aria-label="任务分页"><span>第 {result.pagination.page} / {Math.max(1, result.pagination.totalPages)} 页</span><div className="inline">{result.pagination.page > 1 && <Link className="button small" href={pageHref(result.pagination.page - 1)}>上一页</Link>}{result.pagination.page < result.pagination.totalPages && <Link className="button small" href={pageHref(result.pagination.page + 1)}>下一页</Link>}</div></nav></>}
  </>;
}
