...
export default async function TasksPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const activeFilterCount = ['query', 'status', 'reviewStatus'].filter((key) => Boolean(value(key))).length;
  ...
  return <>
    <header className="tasks-command-header">
      <div>
        <span className="eyebrow">Review operations</span>
        <h1 className="sr-only">内容审核</h1>
        <h2>任务中心</h2>
        <p className="subtle">按批次定位任务，集中处理生成进度、人工审核和交付导出。</p>
      </div>
      <TaskProgressRefresh active={result.data.some((task: any) => task.status === 'pending' || task.status === 'processing')} />
    </header>

    <section className="tasks-context-grid" aria-label="当前任务上下文">
      <Card className="tasks-context-card">
        <CardContent>
          <span className="tasks-context-icon tone-neutral"><PackageOpen aria-hidden="true" size={17} /></span>
          <span><small>当前批次</small><strong>{selectedBatch?.name || '暂无任务批次'}</strong></span>
          <b>{selectedBatch?.statistics.taskCount || 0}</b>
        </CardContent>
      </Card>
      <Card className="tasks-context-card">
        <CardContent>
          <span className="tasks-context-icon tone-blue"><ListFilter aria-hidden="true" size={17} /></span>
          <span><small>当前筛选</small><strong>{activeFilterCount === 0 ? '全部任务' : `${activeFilterCount} 个条件`}</strong></span>
          <b>{result.pagination.totalItems}</b>
        </CardContent>
      </Card>
      <Card className="tasks-context-card">
        <CardContent>
          <span className="tasks-context-icon tone-green"><ArchiveRestore aria-hidden="true" size={17} /></span>
          <span><small>可批量导出</small><strong>本页已就绪</strong></span>
          <b>{exportableCount}</b>
        </CardContent>
      </Card>
    </section>

    <Card className="tasks-filter-panel">
      <div className="tasks-filter-heading">
        <div><Search aria-hidden="true" size={17} /><span><strong>查找任务</strong><small>批次为必选上下文，其余条件可以组合使用</small></span></div>
        <span className="tasks-filter-count">{activeFilterCount === 0 ? '未添加筛选' : `${activeFilterCount} 个筛选条件`}</span>
      </div>
    ...
    <div className="tasks-result-header">
      <div><CheckSquare2 aria-hidden="true" size={18} /><span><strong>任务列表</strong><small>共找到 {result.pagination.totalItems} 条，本页显示 {result.data.length} 条</small></span></div>
      <span>第 {result.pagination.page} / {Math.max(1, result.pagination.totalPages)} 页</span>
    </div>

    {result.data.length === 0
      ? <div className="panel empty-state">没有符合当前条件的任务。</div>
      : <TaskBatchExportForm exportableCount={exportableCount}>
        ...
        <nav className="pagination" aria-label="任务分页">
          <span>第 {result.pagination.page} / {Math.max(1, result.pagination.totalPages)} 页</span>
          <div className="inline">{result.pagination.page > 1 && <Link className="button small" href={pageHref(result.pagination.page - 1)}>上一页</Link>}{result.pagination.page < result.pagination.totalPages && <Link className="button small" href={pageHref(result.pagination.page + 1)}>下一页</Link>}</div>
        </nav>
      </TaskBatchExportForm>}
  </>;
}
