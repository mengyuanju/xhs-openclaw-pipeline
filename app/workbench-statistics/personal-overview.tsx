...
export function PersonalOverview({ filter, onFilter }: { filter: string; onFilter: (filter: string) => void }) {
  ...
  return <section className="panel job-stats-personal" aria-label="我的作业概览">
    ...
    <div className="job-stats-metrics">
      <Metric label="今日新增" value={number(summary?.todayCreated)} />
      <Metric label="累计创建" value={number(summary?.total)} note="包含已废弃作业" />
    ...
    {expanded && <div id="personal-statistics-trend">
      ...
      <p className="job-stats-note">按北京时间统计，独立于列表搜索和页码。审核完成按当前有效审核时间统计，重新生图后可能变化。{summary?.missingDates ? `有 ${summary.missingDates} 项日期不完整，未纳入对应日期统计。` : ''}</p>
    </div>}
  </section>;
}
