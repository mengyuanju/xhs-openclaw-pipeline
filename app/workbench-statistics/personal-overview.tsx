...
import type { Statistics, StateGroup, Summary } from './types';
...
export function PersonalOverview({ period, statistics, onPeriod }: {
  ...
  return <section className="panel job-stats-personal" aria-label="我的作业概览">
    ...
    {!!summary?.anomalies && <p className="job-stats-warning job-stats-personal-warning">待处理异常 {summary.anomalies} 项（含生图失败回审）</p>}
    ...
    {expanded && <div id="personal-statistics-trend">
      <div className="job-stats-heading"><h3>新建与审核完成趋势</h3><div className="job-stats-segments">
        {(['7d', '30d'] as const).map(value => <Button unstyled key={value} type="button" aria-pressed={period === value}
          onClick={() => onPeriod(value)}>近 {value === '7d' ? 7 : 30} 天</Button>)}
      </div></div>
...
export function PersonalStatusFilters({ filter, summary, onFilter }: {
  filter: string;
  summary: Summary | null | undefined;
  onFilter: (filter: string) => void;
}) {
  return <div className="job-stats-chips workbench-personal-state-filters" aria-label="按当前状态筛选个人作业">
    <span className="workbench-personal-state-label">状态分类</span>
