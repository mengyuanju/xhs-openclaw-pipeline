...
export function TaskQualitySummary({
  result,
  onShowImages,
  compact = false,
}: {
  ...
  const hasEvidence = report.issues.length > 0 || report.dimensions.length > 0 || report.limitations.length > 0;
  const allEvidence = <>
    {report.issues.length > 0 && <ul className="workbench-quality-issues">{report.issues.map((issue, index) =>
  ...
  return <section className="workbench-quality-summary" aria-label="当前图片自动质检" data-attention={report.needsAttention} data-compact={compact}>
    <div className="workbench-quality-heading">
      <div><h3>自动质检 · {report.score === null ? '待人工核对' : `${report.score} / 3 分`}</h3>
        <p>{compact ? '系统检测证据，不是人工评分原因。' : '仅针对当前图片版本，最终结果由人工审核确认。'}</p></div>
      {onShowImages && <Button unstyled className="button small" type="button" onClick={onShowImages}>查看图片</Button>}
    </div>
    <p>{report.summary}</p>
    {!compact && report.issues.length > 0 && <ul className="workbench-quality-issues">{report.issues.slice(0, 3).map((issue, index) =>
      <li key={index}><strong>{issue.severityLabel} · {issue.label}</strong><p>{issue.evidence}</p></li>)}</ul>}
    {compact && hasEvidence && <Disclosure className="workbench-quality-evidence">
      <DisclosureTrigger>查看自动质检证据{report.issues.length > 0 ? ` · ${report.issues.length} 个问题` : ''}</DisclosureTrigger>
      <DisclosureContent>{allEvidence}</DisclosureContent>
    </Disclosure>}
    {!compact && (report.issues.length > 3 || report.dimensions.length > 0 || report.limitations.length > 0) && <Disclosure>
