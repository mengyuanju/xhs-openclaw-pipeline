...
- 文案和图片强制复检只允许通过或提交管理员；旧打回、直接废弃及文案快捷直放接口也受后端约束。
...
升级本身不把任何任务改成“待二次分配”，不删除历史内容、不改变负责人、当前版本或原有任务状态。已有待复检任务保留状态，在升级后的质检页面使用新的两种动作。待返工任务继续完成原流程，提交强制复检后使用新规则。已完成和已废弃任务保持原状态。
