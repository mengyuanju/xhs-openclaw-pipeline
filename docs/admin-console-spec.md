...
- 为每个任务固定提示词版本与参考图；图片数量由最终正文在 3–5 张中实时选择，并记录本次实际结果。
...
- `task_configs`：任务固定的提示词版本、模型实际选择的图片数量、审核状态与当前文本修订。
...
- `externalId` 在同一批次中去重；`imageCount` 是旧模板兼容字段，填写时只能为 3–5，但不会固定 Live 交付张数。
- 参考图使用已上传素材文件名，第一版不从用户提供的远程 URL 抓取，避免 SSRF。
- 结构合格行必须在入队前完成 `STRONG | MEDIUM | WEAK | NONE` 判定；Excel 已有判定自动带入，缺失判定在上传预检期间由 OpenClaw 按有界批次自动生成，管理员可在页面复核和修正。
- OpenClaw 只接收完成结构校验的选题、分类和目标用户；工作簿内容一律视为不可信数据。模型必须返回与请求行号一一对应的 JSON，缺行、重复行、额外行、非法档位或超长理由都会使本次导入失败，且不会创建半成品批次。
- 自动判定来源记录为 `OPENCLAW` 并保留实际模型名；常规测试必须使用 Fake，不消耗模型额度。大批次可能触发多次文本模型调用并延长上传时间。
...
- `POST /api/worker-runs` 只接受严格 JSON：`{ max: 1..20, confirmation: "LIVE_MODEL_COST_ACCEPTED" }`，成功返回 `202` 与 `STARTED`、运行 ID、实际处理上限和服务端固定的 `concurrency: 2`。
...
- 网页 Worker 按全局队列顺序处理，单次最多 20 条，最多同时生产 2 条完整任务；并发为 2 时每任务图片并发强制为 1，同一 drain 总模型调用并发不超过 2。状态和失败原因在内容审核页查看，不自动发布。

## Storage Retention Contract

- 同卷生成图片优先使用硬链接同步到审核素材目录；`EXDEV`、权限或文件系统能力限制时回退为独占复制。
- 每次内容尝试结束后保留当前交付、全部人工编辑及其父链、最新尝试完整文件，以及历史尝试的非图片审计文件。
- 只允许删除未被保留集合引用且没有编辑子项的 `GENERATED` 素材；数据库记录先事务删除并写审计，再删除对应文件，失败最多产生可再次清理的孤立文件，不产生悬空数据库引用。
- 全库维护命令默认 dry-run，只有显式 `--apply` 才执行删除和已有文件硬链接去重；任何路径必须受任务输出根或素材根约束。
...
- OpenClaw：Fake 验证需求筛选、生成提示词、参考图与图片编辑参数，不在常规测试消耗额度。
...
1. 管理员能上传示例 Excel，结构合格且未预筛选的行由 OpenClaw 自动判定强/中/弱/无需，页面允许人工复核，并一次性导入至少 1000 条强需/中需任务；筛选未完成不得入队，重复提交不重复建任务。
