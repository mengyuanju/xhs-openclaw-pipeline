...
- 实体替换：一次可建立 1～4 个“产品替换项”，每项绑定自己的真实参考图和使用方式，并可勾选一张或多张源图。每页可选择“只替换一个产品”或“替换框内全部同款产品或特写”：单目标模式要求框住一个完整目标；全部同款模式把人工框作为搜索范围，视觉预检定位最多 4 个匹配实例并返回各自紧框，最终蒙版只合并这些紧框，不把大搜索框内的文字和其他物体一并开放给图片模型。同一张图里的多个替换项会合并为一个原子编辑请求，一次生成、一张联合蒙版和一次逐项结果验收；跨图片则按页建立同一批次，预览全部就绪后可整批采用。“完整产品”模式要求参考图只有一个清楚、完整、遮挡很少的产品；“外观参考”模式允许手部、手腕、裁切或次要产品，但主产品必须清楚且唯一可识别。定位通过后，系统把原图、对应参考图和视觉定位后的联合蒙版交给图片编辑模型；最终再用原图像素覆盖蒙版外区域。生成后核对参考身份、全部目标位置、替换数量、部件拓扑及无关内容。
