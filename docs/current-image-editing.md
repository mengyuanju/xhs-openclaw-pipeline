...
执行机通过租约绑定的远程 HTTP 协议读取冻结上下文、源图和参考图，回传校验记录及最终 PNG；中心复核执行身份、源版本、PNG 尺寸/格式和校验哈希后落库。领取回执、校验提交和结果提交均支持不重放模型的网络重试。中心和图片执行机通过 `imageEditExecutorVersion=4` 协商外观参考模式；版本 3 执行机仍可领取严格模式与历史改图，但不会误领 `APPEARANCE` 任务。
