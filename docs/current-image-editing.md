...
- 实体替换：一次可建立 1～4 个“产品替换项”，每项绑定自己的真实参考图和使用方式，并可勾选一张或多张源图。每页可选择“只替换一个产品”或“替换框内全部同款产品或特写”。单目标模式的人工框只是定位提示：不再检查框是否完整覆盖目标，也不因框内包含持握手部而拦截；系统把完整原图和参考图直接交给图片编辑模型，让模型补全框外的同一产品并保持手部、人物、构图、文字及其他物体不变，之后再做结果验收。全部同款模式仍把人工框作为搜索范围，视觉预检定位最多 4 个匹配实例并返回各自紧框，再用合并遮罩限制修改。同一张图里的多个替换项会合并为一个原子编辑请求、一次生成和一次逐项结果验收；只要其中含单目标项，就使用完整画面直接编辑，避免定位框裁断产品。跨图片按页建立同一批次，预览全部就绪后可整批采用。“完整产品”和“外观参考”的差异继续由生成提示及结果验收约束。
