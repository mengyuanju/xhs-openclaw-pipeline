...
执行机通过租约绑定的远程 HTTP 协议读取冻结上下文、源图和参考图，回传校验记录及最终 PNG；中心复核执行身份、源版本、PNG 尺寸/格式和校验哈希后落库。领取回执、校验提交和结果提交均支持不重放模型的网络重试。中心和图片执行机通过 `imageEditExecutorVersion=9` 协商最新编辑能力：局部修改要求版本 9，旧执行机不能领取；程序标识仍要求版本 8，其他图片模型修改要求版本 7，并会把自动验收未通过的生成结果作为隔离的失败预览上传并保留。局部编辑验收会返回固定失败码和定向修复说明，不再要求修复区域坐标；当文字及无关内容仍完好且修复次数未耗尽时，作业员重新确认费用后可选择以失败预览为编辑目标、以原图为只读外观参照继续补救。失败预览不会自动进入正式图集，作业员仍可查看、对比、从原图重试，也可在明确确认并填写原因后人工采用；系统保留完整审计记录。确定性合成与历史恢复仍兼容版本 3 执行机。
