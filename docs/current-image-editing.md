...
- 添加文字：短句、AI生成预设、可追踪 AI 标识类型、四角/上下/安全区域坐标、字号、颜色、底色、透明度、边距。复用 `applyDeterministicTextOverlay`，浏览器显示即时位置示意；最终图片由 Sharp 确定性叠加。
...
运行机前置条件：现有 Node/Sharp/Codex 环境，以及 PATH 中的 Tesseract 和 `chi_sim`、`eng` 语言包。文字/精确合成/恢复使用本地 OCR，不调用模型。OCR 不可用、低置信度、白名单外文字、必需文字缺失或遮挡都失败关闭，不能采用。当前开发机未检测到 Tesseract，所以真实中文 OCR 尚未人工验收。
...
创建/恢复必须提供 `requestId, sourceImageRunId, sourceAssetId, copyRevisionId, sha256, targetPage`。创建还提供 operation 及对应 overlay/references/mask/instruction；AI 必须提供 `confirmation=LIVE_IMAGE_COST_ACCEPTED`。动作接口必须提供 `requestId, version, reason`。源版本变化返回冲突，无法覆盖新图集。重复 requestId 不重复采用或审计，复用到不同输入会拒绝。
