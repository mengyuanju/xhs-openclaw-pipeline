...
- 实体替换：作业员上传一张真实产品图片，填写被替换物品的可辨识描述，并在原图上框选且只框住一个完整目标。系统先用视觉模型确认“描述、框选和参考图”对应唯一可替换物品；无法唯一定位、框内包含其他受保护物体、目标被截断或参考图不可用时，会在调用图片生成模型之前失败，因此不会产生本次图片生成费用。定位通过后，系统把原图、参考图和目标蒙版交给图片编辑模型，仅允许模型改动框选区域；最终再用原图像素覆盖蒙版外区域，保证同类物品和其他内容不会被误改。生成后另做参考产品身份、外形、颜色、标志、关键细节、单目标替换和背景拓扑一致性校验。
- 局部修改：作业员只在说明中同时写清“改哪里”和“改什么”，例如“把画面右上角的白色水杯改成蓝色”。系统先用视觉模型把说明解析为唯一目标和最小矩形；目标不唯一、说明不具体、目标不完整或必须覆盖文字时，会在调用图片生成模型前失败且不计本次图片生成费用。定位通过后，系统把原图、说明和运行时生成的目标蒙版交给图片编辑模型，再过滤蒙版内的低幅背景漂移并用原图逐像素恢复蒙版外区域。新界面不要求人工填写坐标；历史已创建的遮罩任务仍按原遮罩逻辑执行。
...
执行机通过租约绑定的远程 HTTP 协议读取冻结上下文、源图和参考图，回传校验记录及最终 PNG；中心复核执行身份、源版本、PNG 尺寸/格式和校验哈希后落库。领取回执、校验提交和结果提交均支持不重放模型的网络重试。旧中心缺少 `imageEditExecutorVersion=3` 时，新图片执行机会在注册前停止并提示先升级；旧执行机低于版本 3 时，升级期间不会误领需要正确处理人工生成标识删除血缘的改图任务。
