...
第一版覆盖图片分析、双保留模式、配方创建/发布/归档、分页列表、自动匹配、任务锁定和主图提示词/参考图接入。配方试生成评分与本地信息卡布局渲染留作后续切片，不伪装为本期已完成能力。
...
4. 当前只有主图调用图片模型，第一版视觉配方只接入主图；Sharp 信息卡布局规则暂时只保存，不改变现有模板。
...
最终主图提示词按顺序组合：任务固定的全局 `IMAGE_SYSTEM`、任务锁定的已发布视觉配方、当前 `imagePlan[0].prompt`。有授权保留图时加入 `referenceImagePaths`，否则仅使用提示词。知识库为空、无匹配或模块异常时回退到现有生成行为。
