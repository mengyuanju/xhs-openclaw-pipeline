...
正式文案生成在调用文本模型之前，通过 OpenClaw 执行一次可追溯的网页检索。文本模型只能使用本次研究快照和任务原始参考资料中的事实，最终 `post.sources` 只能引用这两类白名单 URL。
...
- 提供方按 `codex`、`duckduckgo` 顺序尝试。Codex Hosted Search 使用现有 OpenAI/Codex 登录；DuckDuckGo 是无需密钥的后备提供方。
...
- Codex 失败后可由 DuckDuckGo 成功接管，快照保留完整尝试链。
