...
从作业员说明中区分“要修改的目标”和“修改方式”，只制定计划，不执行修改。目标必须唯一确定；用户明确点名的多个对象（如“右下角灰色和浅蓝色两件短袖”）视为一个目标集合，只要整组可唯一识别，candidateCount=1、exactlyOneTarget=true，不要因为对象有两件就当成多个候选。sourceRegion 完整覆盖目标在画面内所有可见部分；目标贴住或超出画面边缘本身不是失败，此时 touchesImageEdge=true，只要修改不依赖无法看见的身份或结构，wholeVisibleTargetInsideRegion 仍可为 true。只有缺失部分确实导致无法可靠修改时 missingPartsRequiredForEdit=true 并 BLOCKED。
...
SUGGEST 时 instructionSpecific 判断补充后的执行说明是否明确，不应仅因原说明简短就置为 false。用户已补充自然修补、文字和边缘保护等必要约束且可以安全执行时，返回 READY，不要重复要求补充同样的约束。删除操作的 destinationRegion 通常为 null；不要把“移除两件衣服”改成移动、缩小或替换衣服，不要扩大删除数量。建议须给出可以直接使用的完整描述，不要只写“建议补充约束”。客户端会以该描述为基础提供精准目标、自然修补、文字与边缘保护三个方向，所有方向保留同一用户要求。
