...
逐项核对 replacements：每个 target.region 中的目标必须只使用其 referenceAttachmentIndex 对应参考图完成替换；STRICT 项核对完整身份与结构，APPEARANCE 项只核对 referenceProductDescription 指定主产品的可见外观和合理补全。不得遗漏、增加、重复或互换产品，不得修改任何框外对象、构图或文字。任一项不确定时 passed=false。
