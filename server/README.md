...
`0021_task_assignment_integrity` 和 `0022_auto_assignment_cursor` 提供任务负责人、显式人员池与历史任务完整性基础。当前 V3 契约下，普通任务先以未分配状态进入全局文案队列，文案执行机可直接领取；文案生成完成、进入待文案审核后，才按待审核额度自动补位或由管理员手工分配。只有加入且启用的普通用户会自动接单，管理员只能手工把任务分给自己，不能进入自动池。显式跳过文案审核的任务是例外，创建时必须指定负责人。V3 工作流复用现有字段，无需新增迁移；仍应按 [迁移说明](migrations/README.md#0021--0022-自动分配与-v3-工作流升级) 完成既有迁移，并先升级中心服务再更新界面。
