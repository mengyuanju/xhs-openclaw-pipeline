...
“修改图片”仍保留独立预览、采用和审计状态，但执行工作由普通图片执行机承担。中心把普通生图和改图放入同一个图片领取与容量裁决中，按既有任务优先级和负责人轮转分派；中心机不需要 Codex 登录，也不会运行改图模型。最新中心返回 `imageEditExecutorVersion=8`：版本 8 增加 SVG + Sharp 程序标识，版本 7 仍是调用图片模型修改的最低领取能力。版本 7 会上传并隔离保存自动验收未通过的生成结果，并支持在作业员重新确认费用后，以冻结的失败图和缩小后的修复区域进行定向补救；原图同时作为只读外观参照，最终仍按完整原始任务验收。作业员也可对比、从原图重试或明确确认后人工采用；版本 3 执行机仍可领取确定性合成与历史恢复。至少一台图片执行机需要启用 `IMAGE_WORKER_ENABLED=true`。
