-- Delivery APIs expose numeric IDs through JavaScript. A PostgreSQL bigint
-- above Number.MAX_SAFE_INTEGER cannot be represented without identity loss,
-- so such an asset reference must never remain in the READY pool.
      ...
      LEFT JOIN assets AS asset
        ON asset.id = CASE
          WHEN COALESCE(selected.image->>'deliveryAssetId', selected.image->>'assetId', '')
              ~ '^[1-9][0-9]{0,15}$'
            AND (
              char_length(COALESCE(selected.image->>'deliveryAssetId', selected.image->>'assetId', '')) < 16
...
WHERE delivery_migration_anomalies.reason IS DISTINCT FROM EXCLUDED.reason;
