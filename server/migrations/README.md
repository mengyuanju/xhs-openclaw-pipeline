...
`0078_image_revision_copy_qc_inheritance.sql` 把“重新生成整套图片”和“仅转换格式 / 背景”创建的图片专用修订版接入 `0067` 的文案质检继承链。只有父版本已经放行、标题/正文/标签完全一致、来源操作为 `REGENERATE` 或 `REPROCESS`，且任务处在允许修改图片的状态时才能继承；普通文案或规划修改仍会关闭生图门禁。

迁移会恢复被该缺陷误送入“待文案审核”、且尚未产生新强制复检的同形任务，同时保留旧 `SUPERSEDE` 审计事件。升级前应停止中心写入并备份 PostgreSQL，升级后确认修复任务进入 `IMAGE_QUEUED`、`copy_qc_released_revision_id` 指向当前版本，并由图片执行机正常领取。

## `0079` 旧中心滚动升级兼容

`0079_legacy_image_revision_inheritance.sql` 处理数据库已升级、中心进程尚未重启的短暂窗口。新版中心仍在切换任务版本前显式写入继承记录；旧中心若遗漏该写入，数据库只会为已放行父版本、文案字段完全一致、来源为可信 `REGENERATE` / `REPROCESS` 的图片专用版本补记相同继承链。任何普通规划修改、文案修改、未放行版本或非图片队列状态仍按原规则进入文案复检。

`0088_initial_baseline_null_manual_review.sql` 修复机器文案结果自带 `manualReview: null` 时被 `0086` 误认为人工标注的问题。仅从有同任务成功 COPY 执行记录、未改动且未清理的根 `GENERATION` 版本补录初稿基线，并修正后续捕获规则；非空人工标注仍被排除。迁移只补基线，已受阻的二次分配处置单仍需由管理员执行“重试还原 / 清理”。

## `0080` 文案质检问题标签

`0080_copy_qa_reason_tags.sql` 增加质检员个人问题标签和管理员公共标签审核状态。个人标签默认只进入创建人的选择器；公开申请通过后才会供全体质检员复用。标签用于打回时，中心会在返工修订版和审计事件中保存当时的中文快照，以后停用标签不会破坏历史记录。

新 Web 的标签写入会检查 `/health` 中的 `copyQaReasonTagsVersion=1`。发布时先备份数据库并应用迁移，再同步切换中心与 Web；旧中心不会接收新标签写入。

## `0081` 标准表双 Query 与任务ID拆包

`0081_query_package_issued_query.sql` 只为 `query_package_items` 新增可空的 `issued_query`，保存原始下发内容。现有 `query` 继续保存实际作业内容：优先生产query，空白时使用下发query。历史记录的下发内容保持空值，不从生产内容猜测回填。

XLSX 标准表按列名识别“下发query”“生产query”“任务ID”，可选“序号”映射现有 `external_id`。任务ID沿用甲方批次编号的 32 位十六进制格式。一次上传中的不同任务ID自动拆为多个词包；名称追加对应批次编号。每个 Excel 数据行保留为一条记录，单元格内换行不拆分；重复判断只在同一任务ID内进行，重复记录保留但不可投产。“是否进入生产”不替代既有人工筛选。

标准表存在缺失任务ID、非法格式、公式或无效 Query 时，阻止整次创建并报告行号。所有拆分词包在同一数据库事务中创建，一次 requestId 对应完整结果，失败回滚、重试不重复建包。旧文本、TXT、单列 CSV 和单列 XLSX 入口继续兼容。

先应用迁移并启动新版中心，再更新 Web。新版中心报告 `queryPackageVersion=7`；新版 Web 的创建和 XLSX 预检会拒绝连接旧版中心，避免静默丢失下发内容或任务ID映射。

## `0084` 账号文案抽检比例

`0084_account_copy_sampling_rate.sql` 为 `app_users` 增加可空的 `copy_sampling_rate_bps_override`（0–10000，`null` 继承生产配置），增加配置变更审计和冻结比例来源、账号版本快照。历史账号保持继承，累计余数和已冻结成员不变；历史强制复检补充 `MANDATORY_RECHECK` 来源标签。

普通抽检仍受全局总开关控制；开启时账号比例优先于全局默认值，0% 仍在结批或超时后对非空尾批保底抽一条，返工强制复检始终全检。先应用迁移并启动报告 `copySamplingVersion=2` 的中心，再发布 Web。旧 Web 不传新字段时不会清空已有覆盖；新版 Web 代理向旧中心写入新字段时会拒绝，避免静默丢失配置。回滚中心代码会恢复按全局比例处理新冻结，须由管理员知晓该行为，新增字段和审计表应保留。

## `0085` 质检共享队列

`0085_shared_quality_queues.sql` 停止为文案、图片质检待办自动选择处理人，并清空存量 `PENDING` 项的预分配。具备对应质检权限的账号共享可处理队列，普通质检人员仍不能处理自己最终审核的文案或自己提交的图片；管理员保留既有完整视图和例外处置能力。旧分配字段暂留作历史兼容，但不再参与列表可见性或操作授权。
