...
function qaItemFrom(row, actor) {
  ...
  const common = {
    ...
    capabilities: {
      canPass: Number(row.final_approver_account_id) !== actor.userId && row.status === 'PENDING',
      canReturnSingle: Number(row.final_approver_account_id) !== actor.userId && row.status === 'PENDING',
