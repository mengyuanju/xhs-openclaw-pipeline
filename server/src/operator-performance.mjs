...
export const PERFORMANCE_EVENTS_SQL=`SELECT e.*,EXISTS(SELECT 1 FROM tasks t WHERE t.id=e.task_id) AS task_exists,
    ...
    CASE WHEN e.kind='RETURN' OR e.kind='QUALITY' AND e.data->>'outcome'='RETURN' THEN
      (SELECT count(*) FROM operator_performance_events r WHERE r.task_id=e.task_id AND (r.occurred_at,r.sequence_id)<=(e.occurred_at,e.sequence_id)
        AND (r.kind='RETURN' OR r.kind='QUALITY' AND r.data->>'outcome'='RETURN') AND r.data->>'exclusion' IS NULL) ELSE 0 END AS return_round
...
const CURRENT_SQL=`SELECT latest.*,t.query,t.production_batch_id AS batch_id,
...
export async function readOperatorPerformance(pool,actor,input={},options={}) {
  ...
  if(filters.snapshotToken) {
    ...
    if(!options.kind || options.kind==='report') {
      ...
      for(const key of ['period','accountId','stage','batchId','query']) if(input[key]!==undefined && filters[key]!==original[key]) {
    ...
    try {
      ...
      const events=assertComplete((await client.query(PERFORMANCE_EVENTS_SQL,[start,end,asOf,filters.accountId,filters.stage,filters.batchId])).rows).map(eventFrom);
      const pending=assertComplete((await client.query(PENDING_SQL,[start,end,filters.accountId,filters.stage,filters.batchId])).rows);
      ...
      const current=assertComplete((await client.query(CURRENT_SQL,[filters.accountId,filters.stage,filters.batchId])).rows).map(row=>({
        ...
        phase:row.phase,state:row.state,query:row.query,batchId:number(row.batch_id),at:iso(row.waiting_at),waitingMs:Math.max(0,Date.parse(asOf)-Date.parse(row.waiting_at))}));
  ...
  if(options.kind==='detail' || options.accountId!==undefined) {
    ...
    for(const day of report.trend) {
      const daily=all.filter(row=>String(new Date(Date.parse(row.at)+8*3_600_000).toISOString()).slice(0,10)===day.date);
      const summary=summarizeOperator(daily);
      trend.push({date:day.date,submitted:summary.submitted,returned:summary.returned,COPY:summary.COPY.firstPass,IMAGE:summary.IMAGE.firstPass});
    }
    return {person,range:report.range,asOf:report.asOf,snapshotToken:snapshot.token,trend,
      ...
      timeline:snapshot.timeline.filter(row=>tasks.has(row.taskId)),current:snapshot.current.filter(row=>accountId===null || row.accountId===accountId),
