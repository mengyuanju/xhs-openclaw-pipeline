...
test('personal QA history uses the authenticated identity for users, reviewers and administrators',async()=>{
  let role='USER',version=1,calls=0;
  const storageRoot=await mkdtemp(join(tmpdir(),'personal-qa-http-'));
  const app=createControlPlaneApp({storageRoot,enforceUserAuth:true,repository:{
    ...
    personalQualityActivity:async(actor)=>{calls++;return {accountId:actor.userId};},
  }});
  const server=app.listen(0,'127.0.0.1');await new Promise(resolve=>server.once('listening',resolve));
  const url=`http://127.0.0.1:${server.address().port}/v1/personal-workspace/qa-activities`;
  const headers=()=>({'X-Actor-User-Id':'10','X-Actor-Username':'test','X-Actor-Role':role,'X-Actor-Credential-Version':'1'});
  try {
    for(role of ['USER','REVIEWER','ADMIN']) {
      const response=await fetch(url,{headers:headers()});assert.equal(response.status,200);
      assert.match(response.headers.get('cache-control'),/no-store/);assert.equal((await response.json()).data.accountId,10);
    }
    version=2;assert.equal((await fetch(url,{headers:headers()})).status,401);assert.equal(calls,3);
  } finally {await new Promise(resolve=>server.close(resolve));assert.ok(storageRoot.startsWith(join(tmpdir(),'personal-qa-http-')));await rm(storageRoot,{recursive:true,force:true});}
});
