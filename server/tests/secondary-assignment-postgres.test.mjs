...
test('secondary assignment migrates existing states and preserves QA subjects while resetting content', {
  ...
  try {
    ...
    async function fixture({initial=true,mandatory=true}={}) {
      ...
      let original=null;
      if(initial)original=Number((await db.query(`INSERT INTO copy_revisions(task_id,revision,content,revision_origin) VALUES($1,1,$2,'GENERATION') RETURNING id`,[task,baseline])).rows[0].id);
    ...
    const legacy=await fixture(),missing=await fixture({initial:false}),ordinary=await fixture({mandatory:false});
    ...
    assert.deepEqual((await db.query('SELECT content FROM copy_revisions WHERE id=$1',[reset.current_copy_revision_id])).rows[0].content,baseline);
    ...
    const report=await repo.operatorPerformance(admin,{});assert.equal(report.metricVersion,5);assert.equal(report.summary.COPY.qualityOutcomes.judged,3);
    ...
    // IMAGE escalation cancels edits, destroys asset payloads/files and preserves facts.
    ...
    await assert.rejects(repo.discardImageQaItem(imageItem.public_id,{requestId:randomUUID(),note:'不能直接废弃'},{actor:qa}),/强制复检/);
    const imageReceipt=await repo.escalateQualityToAdmin('IMAGE',imageItem.public_id,{requestId:randomUUID(),expectedRevisionToken:'b'.repeat(64),note:'仍有图片错误'},{actor:admin,storageRoot:storage});
    assert.equal((await repo.getReassignmentCase(imageReceipt.caseId,{actor:admin})).canAssign,true,JSON.stringify((await db.query('SELECT * FROM task_reassignment_cleanup WHERE case_id=$1',[imageReceipt.caseId])).rows));
    await assert.rejects(access(file),{code:'ENOENT'});
    assert.equal(await repo.getAsset(Number(asset.id)),null);
    assert.equal((await db.query('SELECT result FROM image_runs WHERE id=$1',[run])).rows[0].result,null);
    assert.equal((await db.query('SELECT count(*) FROM image_edit_requests WHERE task_id=$1',[imageTask.task])).rows[0].count,'0');
