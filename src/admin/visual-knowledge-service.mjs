...
export async function analyzeVisualImage({ buffer, mimeType, fileName: _fileName, vision, modelApi }) {
  ...
  try {
    ...
    const result = await client.runVision({ prompt: businessPrompt('VISUAL_KNOWLEDGE_ANALYSIS_SYSTEM', { contract: internalPrompt('INTERNAL_VISUAL_KNOWLEDGE_OUTPUT') }), inputPaths: [normalizedPath] });
    return {
