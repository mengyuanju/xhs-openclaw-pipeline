...
- [ ] Task 23: 定义并测试有界批量需求检测契约，严格校验模型 JSON、行号覆盖和四档枚举。
- [ ] Task 24: 将自动检测接入 Excel 预检，记录 `OPENCLAW` 来源与模型名，保留 Excel 判定和人工复核。
- [ ] Task 25: 更新导入界面与文档，完成全量测试、类型检查、构建和浏览器验证。

### Checkpoint: OpenClaw Demand Screening

- [ ] 未预筛选的结构合格行全部经过 OpenClaw，已有 Excel 判定不重复调用。
- [ ] 任一批次模型输出不完整或非法时不创建导入批次。
- [ ] 页面展示自动判定并允许管理员修改后再入队。
