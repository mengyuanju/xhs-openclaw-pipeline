# 当前实施：布局模板库与模型视觉规划（2026-09-07）

用户已授权修改，模板库、模型视觉规划、自动入库、版本保护和失败恢复已完成。完整方案见 [布局模板库、模型视觉规划与自动入库](../docs/layout-visual-catalog-plan.md)，结果见 [实施与验收](../docs/layout-visual-catalog-acceptance.md)。

已将参考图整理为 10 类、27 个版本化模板，区分版式分类与现有页面用途；在现有视觉规划调用中按内容选模板并细化设计，生图前自动保存完整规划。复用中心和本地 JSON 存储，本地新增任务规划状态列。全量及定向回归、类型、构建、浏览器和模拟 Smoke 通过；未消耗模型额度或部署远端。下方授权和完成状态均属于历史任务。

---

# 历史方案：提示词统一管理与视觉规划开关（2026-09-07）

用户已授权完整实施，六组功能及无额度回归、浏览器和小样本真实图像修复验收已完成。最终结果见 [实施与验收](../docs/prompt-governance-acceptance.md)。管理员实际请求展示另见 [功能说明](../docs/model-request-visibility.md)。完整方案见 [提示词治理方案](../docs/prompt-governance-plan.md)。

用户要求进一步说明隐藏业务规则如何修改，细则见 [业务规则人工管理](../docs/prompt-business-rules-detail.md)。该细则用于说明治理范围，已更正“正则改写”为实际的标记截断与固定规则追加。

核心决策：复用中心现有提示词版本、JSON设置与执行快照；已确认图文锁定；视觉规划可开关且只设计画面；管理员可编辑业务规则，技术约束与实际脱敏请求仅在管理员后台只读展示，普通用户和审核员的页面及接口均不开放请求明细。中心补充权限、发布校验与知识分析薄接口，没有新增中心数据库迁移或修改调度协议；执行机增加快照接线，共享业务模块需随节点升级。

当前任务清单见本目录 todo.md 顶部。下方历史计划中的授权和完成状态属于旧工作，不作为本轮实现授权。
...
## 2026-09-05: 本机生成链路性能与恢复优化

用户已确认按图片接收 → 视觉规划 → 检索 → 按需并发 → 时间预估顺序实施。
基线：HEAD 26ddbd6，702 项本地回归通过；此前真实样本中规划三次共 271 秒，检索 68 秒，三张图总计 947 秒。旧样本不可当成当前 Codex 链路的速度承诺。

1. P1 图片接收：隔离调用输出目录，读取实际产物路径，完整解码并验证来源；重复事件去重，多候选歧义保留而非猜测。仅接收/落盘失败不得触发生图重放。测试不同文件名、重复事件、歧义、旧文件、越界及损坏图片。
2. P2a 规划契约：共享首次与局部修复流程，规划和修复显式沿用配置思考等级；保存失败输出、固定有效页面，只请求修复失败页面；最终仍做完整事实与可见文字校验。
3. P2b 结构化输出：为规划提供实际业务 JSON Schema，Codex 的输出约束不再仅覆盖 rawText 外壳；其他适配器保留兼容契约与应用层校验。结构来源为现有视觉/版式契约，不新增模型或授权方式。
4. P3 检索：有充分摘要和来源片段即停止无意义的权威词补搜；保留来源安全校验与失败边界，检查任务内快照复用，记录请求次数和耗时。不增加跨任务长期缓存或默默降低来源要求。
5. P4 并发：不删除旧串行锁或放开现有 Codex 限流；记录排队耗时，缺乏实际竞争证据时保留上限。
6. P5 预估：按执行器、模型、思考等级、页数记录近期本地阶段耗时，用完成/剩余阶段及实际并发情况估算，样本不足保留明确的冷启动估计；不把阶段百分比当线性耗时。

边界：不修改中心/分布式服务，不启动生产任务，不消费模型额度；复用现有本地输出和检查点，密钥及参考图不进入新诊断记录。每片先红测试再实现、回归并独立提交。最终执行全量测试、类型检查、构建和必要浏览器检查；单独记录尚未进行的真实模型速度验收。

资料：Codex 结构化输出遵循官方 https://learn.chatgpt.com/docs/non-interactive-mode 的 --output-schema 契约；OpenClaw 输出按本机安装源码 outputs[].path 接收，不猜测新的提供方字段。

### 本轮实施与验证记录

- P1 已独立提交：接收实际 PNG、原生重复事件去重、歧义候选留存，独立测试发现并修复部分附件转换失败后的副本清理问题。
- P2 已接入两条入口，共用业务 Schema 与逐页校验/修复，保留有效页与脱敏失败响应；补测缺页、畸形修复、根元数据保护及明确文字白名单。
- P3 DeepSeek 完整多站点证据一次结束，补搜与显式权威要求保留；修正尾点域名去重及伪 gov/edu 域名评分。现有检查点复用测试继续通过。
- P4 只增加 queueWaitMs，不提高文本或图片并发许可。P5 按阶段与同配置历史中位数预估，恢复样本排除，无新心跳时的网页轮询也识别阶段超时；不同输出根目录默认隔离历史，不改执行机/中心存储配置。
- 最终 fake 全量回归 758/758 通过（约 21.7 秒）；类型检查和生产构建通过。隔离本机网页验证历史样本提示、冷启动提示、旧记录及超时 unknown，浏览器无警告/错误；测试页与服务已关闭。
- 实施说明及边界：docs/local-generation-optimization.md。未重启生产执行机，未调用真实模型；没有优化后真实生成时长或图片质量的测量结论。

## 2026-09-05: 执行机文案与生图并发队列（方案及实施记录）

本节响应“通过 env 分别控制文案和生图任务最大并发，容量未满时自动领取缺额并并发执行”。下方保留原方案；现已完成执行池、中心批量契约与 Codex 许可实现，本机真实模型效率复测已完成，中心运行环境未变更。

### 目标与配置

每个执行机进程维护 COPY、IMAGE 两个独立的有界执行池；中心按 nodeId + kind 约束已领取且仍为 RUNNING 的执行总数。任务上限由执行机 .env 决定，启动时校验并注册到中心，修改后重启生效。

```dotenv
# 示例：文案最多 3 个任务，图片最多 2 个任务
...
- 两个新并发变量未设置时均为 1，兼容现有行为；建议校验为 1–32 的整数，空值、0、负数、小数和非法字符串直接启动失败。32 是方案建议的应用输入边界，不代表模型服务可承受的并发。
- IMAGE_WORKER_ENABLED=false 时图片有效容量为 0，不调用图片领取接口。继续使用现有开关，不用并发值 0 表示停用。
- 一个生图任务包含该任务的一整套图片。EXECUTOR_IMAGE_CONCURRENCY 不等于任务内逐页并发 XHS_IMAGE_CONCURRENCY；旧本地 worker 的 XHS_TASK_CONCURRENCY 也不复用。
- .env 是容量配置来源；中心保存的是执行机上报的容量，用于原子领取校验。文案归属、人工审核和生图重试规则保持现有语义。

### 现有代码中必须处理的限制

| 位置 | 当前行为 | 修改方向 |
| --- | --- | --- |
| src/executor/cli.mjs 的 runLane | 每通道 await 一次完整 runOnce，再开始下一次 | 使用共享执行池调度器，按空闲容量补领 |
| src/executor/deepseek-simulator-cli.mjs | 复制了正式入口的串行循环 | 复用同一配置解析和调度逻辑 |
| server/src/postgres-repository.mjs 的 #claim | 节点已有任意同类 RUNNING 即返回 null；LIMIT 1 | 改为数量校验及有界批量领取 |
| src/executor/agent.mjs 的 pendingFailures | Map 以 COPY/IMAGE 为键，每类只能保留一条失败 | 以 executionId 为键，逐执行独立回报与重试 |
| src/openclaw.mjs 的 runTextExclusively | 模块内文本调用硬串行 | 需要真实文本模型并发时参数化此限制 |
| src/codex-runtime.mjs | 总许可只能为 1/2，图片许可硬编码为 1 | 需要真实多任务 Codex 生图并发时参数化模型调用许可 |

任务队列并发和模型调用并发需要分别验收。只修改领取循环无法突破中心限制；只修改任务队列也不会自动突破模型适配器内部限制。

### 执行池调度

新增共享 src/executor/scheduler.mjs，保存每类任务的 activeExecutions、正在领取的预留槽位、补领状态和重试时间。activeExecutions 以 executionId 为键，涵盖执行、上传以及尚未确认的失败回报；它与预留槽位不重复计数。

```text
空闲槽位 = max(0, 本地并发上限 - 活跃执行数 - 领取中的预留槽位)
中心可领数 = max(0, 节点注册上限 - 中心同类 RUNNING 数)
实际领取数 = min(请求数量, 中心可领数, 当前符合条件的任务数)
```

1. 启动通过就绪检查、注册后，两个已启用执行池各自补满。
2. 同一执行池仅允许一个补领过程；在发送领取请求前预留空闲槽位，避免定时器与完成回调重复领取。
3. 收到任务后，先全部纳入执行池，再启动各自的异步执行。只释放没有领到任务的多余预留槽位。
4. 每个任务独立执行、记录结果。一个任务结束并完成必要回报后释放一个槽位，立即触发该池补领，不等待整批任务全部结束。
5. 无任务、暂时性错误、中心容量暂满时，按 EXECUTOR_POLL_MS 等待重试；已有任务继续执行。轮询计时器要能在有空槽且到期时补领，不能等所有活跃任务结束才检查队列。
6. 共享模型认证、额度或限流暂停时暂停新领取，并沿用现有恢复规则；不通过重跑任务重试状态回报。
7. SIGINT/SIGTERM 停止新补领，已发出的领取请求返回后其任务仍纳入收尾处理；等待在途执行及回报收尾期间继续节点心跳。
8. --once 保持每种已启用类型最多尝试执行一个任务的兼容语义，不补领，避免并发配置改变一次性命令的工作量。

例如 COPY 上限为 3：启动领取 A/B/C；B 先完成，立即补领 D，此时 A/C/D 同时在池中；始终不超过 3。若中心只有 1 条任务，先执行该条，剩余容量持续轮询。

### 中心服务与客户端

建议新增 claim-copy-batch、claim-image-batch 两个接口，保留原单条接口的返回结构。新接口请求为 { nodeId, limit, requestId }，响应为 { requestId, claims: [] }；limit 表示这次请求的空闲槽位数，不能决定节点上限。

- 节点注册/心跳增加 copyConcurrency、imageConcurrency。旧节点首次注册默认 1；已有节点请求中省略容量字段时保留原值，避免无关注册覆盖配置。
- 新增增量 SQL 迁移：节点两个容量字段、RUNNING 执行的 node_id/kind 条件索引、领取请求回执表。schema.sql 是有校验和的不可变基线，不能直接修改；实施时取下一个可用迁移编号。
- 在同一个短事务中锁定 executor_nodes 节点行，读取中心 RUNNING 数，计算可领数，再用 FOR UPDATE SKIP LOCKED LIMIT $n 锁定候选任务。对每个任务创建独立 executionId、配置快照及必要的 image_run，最后提交。
- 原单条接口复用同一领取核心，内部请求数量为 1，继续返回单条/null；所有入口均遵守中心容量限制。
- 保留文案仅领取指定 copyExecutorNodeId 的条件；图片保留原节点重试/恢复绑定、5 秒冷却及现有排序，不能以并发改造绕过这些条件。
- 不在领取事务中调用模型或上传文件。任意一条领取持久化失败，整批事务回滚。
- requestId 用于领取幂等：回执与执行记录同事务保存，以 nodeId + kind + requestId 唯一；同 ID 重试读取原回执，不额外领取。同 ID 参数不一致应拒绝。正常下一轮轮询使用新 ID，包括上轮空队列的情况。
- 网络超时不能假定“没有领到”。执行机保留该请求的槽位及 requestId，使用同 ID 对账重试；收到原执行列表后按 executionId 去重，已终态执行不能再次启动模型。
- 中心的 RUNNING 计数涵盖进程重启遗留任务，不能因为本地 Map 为空而超领。第一版继续按现有人工恢复机制处理崩溃遗留执行，不增加未经实现验证的自动租约回收。
- health 新增并发领取能力标志。新执行机开启新调度器前检查能力，不把连接旧中心后仍串行运行误报为并发生效。

### 执行隔离与失败处理

- 将领取和执行已领取任务拆为清晰接口；保留 runCopyOnce/runImageOnce 作为兼容包装。调度器不能并发调用仍带有按 kind 共享失败状态的旧实现。
- pendingFailures 改成 Map<executionId, { kind, claim, error, retryAt, reporting }>。回报重试每个 executionId 最多一个在途请求。
- 失败回报未确认时继续占用对应任务槽位；其余任务与其余空闲槽位可继续工作。回报成功或中心确认 STALE_EXECUTION，且本地执行链已退出，才释放槽位。
- 图片自动重试仍按整次执行计数；网络回报重试不会创建新执行或重复增加失败次数。保留 Codex 错误 autoRetry:false 规则及图片上传检查点。
- 每个任务使用独立执行上下文、日志和输出目录。现有 AsyncLocalStorage 跟踪和整数 taskId/executionId 路径可以复用，但增加多任务交错的回归验证。
- 所有异步执行都捕获异常并收尾；单个失败不能结束整个调度器。成功上传/完成回报的现有恢复流程需做并发回归，不能为补槽删除其他活跃执行仍使用的检查点。

### 模型调用层配套（按所用提供方实施）

若验收包含“多个任务真正同时调用生成模型”，应一并实施下列配套；否则只能承诺多个任务并发推进，模型阶段仍可能排队。

- OpenClaw：把模块内文本互斥队列改为有界共享信号量，建议新增 XHS_OPENCLAW_TEXT_CONCURRENCY，默认 1；各调用继续使用独立 sessionId。不得为每个 client 分别建一个看似共享的上限。
- Codex：保留现有 SQLite 跨进程许可机制，扩展 XHS_CODEX_CONCURRENCY 的有效范围并新增 XHS_CODEX_IMAGE_CONCURRENCY（默认 1），要求图片调用上限不大于总调用上限。总调用默认仍为 2。
- 同一 runtime 数据库的所有使用进程必须使用一致的容量配置；保留额度/认证暂停、进程树结束后释放许可、崩溃许可回收和排队超时规则。部署时同步相关进程配置，不能只改一个调用方造成共享限制不一致。
- Codex 单个任务逐页串行可以保留；让两条任务各运行一个生图调用即能实现跨任务图片并发，无需同时放大单任务内部并发。
- 举例：图片任务上限 2，但模型图片许可仍为 1，则两个任务可同时规划/上传，真实生图调用仍最多 1 个。要同时生图 2 个，模型图片许可也要配置为 2，且总许可至少为 2。此例不承诺外部账号吞吐或耗时提升。

### 实施顺序与验收

按 tasks/todo.md 的 C1–C10 顺序和依赖实施：配置契约 → 中心容量与批量领取 → 客户端 → 单任务隔离 → 执行池 → CLI 接入 → 提供方许可配套 → 回归及说明。

实现验收：COPY=3、IMAGE=2 时并发峰值分别达到且不超过 3/2；短任务结束即补位；余量不足、空队列后新任务到达都能继续领取；单条失败与回报中断不影响其他执行；同时领取不会重复或超限；响应丢失重试不会多领；图片关闭零领取；--once 保持原工作量；旧节点缺省为 1。自动化回归使用 fake；用户追加授权的手动性能测试使用真实模型。

中心事务的争抢正确性应使用隔离 PostgreSQL 测试数据库验证，不能只以 SQL 字符串匹配或 fake 查询断言声称并发安全。不得连接生产数据库做争抢测试。

预期检查：新增定向 node:test、npm test、npm --prefix server test、npm run typecheck。不修改 UI 时不增加浏览器验收；模型配套另以 fake 记录真实 runner 的并发峰值和额度暂停传播。

部署顺序为先中心增量迁移和兼容 API，再更新执行机；默认 1/1 验证兼容后按 env 配置目标容量。降低容量不强杀在途执行，中心只在 RUNNING 数低于新上限后放行新任务。应用回退应保留已执行迁移文件，符合现有校验和机制。本轮交付实现及本机测试，不进行中心部署。
...
## 2026-09-06 作业列表排版与具体作业员筛选

用户要求修复列表文本拥挤，并搜索具体作业员的作业内容。复用当前管理员全部作业入口和既有 createdByUserId 精确筛选接口，不修改中心生成流程。人员选择器按姓名或账号搜索当前用户（含停用账号），显示账号区分重名，和角色、状态、Query 组成交集。Query 保持选题关键词语义，全文通过现有详情查看。

列表将 11 列组合为 6 个信息分组，固定信息列宽、文本预览行数与右侧操作列；桌面容器独立滚动，平板和手机使用卡片。验收涵盖长 Query/节点名称、同名作业员、组合筛选、翻页后换人回首页、错误重试、清除、320/768/1024/1440 宽度。测试使用模拟数据，不消耗模型额度。

验证完成：根目录 808/808，类型检查、生产构建通过；既有两条 UI 源码断言同步为分组列。浏览器使用 257 条模拟作业，确认同名人员按账号区分、停用人员历史记录、51 条指定账号任务跨三页、人员/角色/状态/Query 交集、换人回第一页、清除筛选、长 Query 三行预览和完整详情。320/768/1024 使用卡片，1440 完整桌面列，均无页面横向溢出；旧文案列表的查看/重试/废弃操作在桌面和手机均正常排版。

独立审查指出异常人员数据可能在渲染期报错；已通过 loadJobCreators 在 Promise 内校验数组和字段，新增回归覆盖 null、分页对象、缺字段、251 条同名/停用名单。真实浏览器验证 HTTP 失败和异常 200 数据均能提示并重试恢复，最终控制台无警告或错误。真实 Next/中心代理验证 ADMIN 可查目录及指定账号，REVIEWER/USER 不能读取目录，USER 伪造账号筛选仍只返回自身任务。日志和隔离验证脚本存于 .codex_artifacts/admin-creator-*，预览已停止，保留原本运行的本地中心和 Web。

## 2026-09-06 列宽、分页与操作响应优化

- 用户要求缩窄 Query 列，页脚支持首页/尾页/页码跳转，优化日常操作并保持响应速度。
- 实施：桌面 Query 约占四分之一；分页增加 20/50/100 条、页码跳转和区间总数，保留页脚、加载期间保留旧表高度并禁用旧行操作；增加清空筛选。
- 请求：测量管理员健康检查与任务读取串行耗时，改为并行且继续验证能力；切页中止旧请求、隐藏页面暂停自动轮询。继续服务端分页，不缓存任务结果或预取全量。
- 验证：边界/非法跳页、页大小/筛选重置、快速切换、失败重试；257 条隔离数据，320/768/1440/1920 布局，受控延迟前后对照、类型/测试/构建。

交付验证：812/812 根目录测试、类型检查和生产构建通过。使用隔离生产预览确认 257 条任务、20 条尾页 241–257、50 条尾页 251–257、100 条尾页 201–257；页码 Enter 跳转、越界错误、改条数回首页、筛选清空、错误页脚重试。慢请求期间旧表保持高度并设 inert，翻页成功后列表顶部定位到 76px；失败明确显示上次范围和失败目标页。1920px 下表格 1565px、Query 391px（25%），320px 页脚无横向溢出；旧慢查询与新角色筛选交错时新筛选正常生效。

性能证据：同样每次读取模拟延迟 100ms，管理员分页读取从串行约 214ms 降为并行约 109ms，仅是受控对照，不代表真实中心网络时延。任务继续按页请求，不缓存任务结果或预取全量；隐藏页面暂停自动轮询，回到可见时刷新。取消仅覆盖浏览器 fetch，不承诺取消代理到中心的查询或正在执行的 SQL。

独立审查发现并行后的能力门禁提示可能被挂起/失败任务请求掩盖，以及分支失败未取消兄弟请求；均已修复并复核通过。deferred 回归证明健康检查不支持时立即提示升级，任务拒绝被提前观察；每轮 finally 取消局部控制器，不影响新一轮。日志位于 .codex_artifacts/pagination-*。
