# 布局模板库与模型视觉规划（2026-09-07，实施完成）

- [x] 对照参考图片核实当前模板、随机选择、模型规划和持久化链路，保存可行方案。
- [x] A. 版本化目录契约及 27 个模板。
  - 依赖：无。验收：分类/编码完整，旧版左右语义保持，新版按截图解释。
  - 预计文件：新目录契约、初始目录 JSON、目录测试；范围 S–M。验证：目录/兼容契约测试。
- [x] B. 原有设置的导入与原子更新。
  - 依赖：A。验收：重复导入幂等，无效批次不写入，并发编辑不丢失其他设置。
  - 预计文件：导入服务、production-settings、中心 repository、本地 settings store、导入测试；范围 M。验证：SQLite 和 fake PostgreSQL 导入测试。
- [x] C. 后台模板表格与 JSON 导入。
  - 依赖：B。验收：27 项可筛选、编辑和启停，导入结果清晰。
  - 预计文件：布局设置 UI、导入 API、中心路由、权限路由、UI 验证；范围 M。验证：权限接口、类型检查、真实浏览器。
- [x] 检查点 1：目录、导入和现有自定义布局兼容通过，复核整体方案范围后继续。
- [x] D. 自动选择改为模型按内容规划。
  - 依赖：A、B。验收：人工指定优先，自动模式不再提前随机锁定，关闭规划零模型调用，原文字不变。
  - 预计文件：image-layout-controls、visual-plan-schema、visual-plan、visual-plan-generation、规划测试；范围 M。验证：fake 模型返回、错误模板和原文锁定测试。
- [x] E. 主流程和独立生成接入。
  - 依赖：D。验收：同一三页样本传递冻结目录、所选版式和统一风格，模板设计实际进入生图提示词及布局验收。
  - 预计文件：pipeline、standalone-image-generation、image-prompt、image-alignment、集成测试；范围 M。验证：无额度集成测试。
- [x] F. 完整规划提前入库与失败保留。
  - 依赖：E。验收：生图前已持久化完整规划，失败可查询，完成写回不覆盖先存字段；写入检查运行归属。
  - 预计文件：执行器运行模块、中心 API、repository、中心客户端、持久化测试；范围 M。验证：保存/失败/完成和权限测试。
- [x] G. 版本能力、续跑与详情展示。
  - 依赖：F。验收：旧执行机不领取不支持的目录，新配置不改变续跑，详情可查所选模板及版本。
  - 预计文件：能力协商模块、中心领取逻辑、恢复模块、任务详情、兼容测试；范围 M。验证：旧快照与恢复测试、浏览器检查。
- [x] 检查点 2：三页完整链路和失败恢复通过，确认目录更新不会影响进行中的任务。
- [x] H. 模型生成模板候选入口。
  - 依赖：B、C。验收：模型结果走同一导入校验，自动保存未启用候选，不修改任务原文或自动污染活动目录。
  - 预计文件：候选服务、API、后台入口、可管理提示词、fake 返回测试；范围 M。验证：有效/无效/重复模型结果和权限测试。
- [x] 完成检查：主工程全量 967/967、中心全量 159/159 及后续新增定向回归、类型检查、构建和浏览器通过。详见 [实施与验收](../docs/layout-visual-catalog-acceptance.md)；本轮未调用真实模型，图片质量不作已通过结论。

---

# 历史任务：提示词治理与视觉规划开关（2026-09-07，方案与实施）

- [x] 盘点生成、规划、审核、修复、检索、知识库和适配层提示词，以及程序对人工规则的隐式覆盖。
- [x] 核实中心可直接复用新 prompt kind、JSON设置与现有执行快照；列出本地SQLite和中心辅助分析的边界。
- [x] 编写 [优化方案与拟改文件清单](../docs/prompt-governance-plan.md)，保留原有业务代码和提示词发布状态。
- [x] 补充 [隐藏业务规则修改细则](../docs/prompt-business-rules-detail.md)：逐阶段现状、具体改法、配置边界与验证例子。
- [x] 根据用户确认限定展示范围：完整脱敏请求、程序追加内容与技术约束仅管理员后台可见，页面和接口均需鉴权；已完成页面/API权限验收。
- [x] 用户已明确授权完整方案实施与测试；六组功能已完成，未自动部署或发布生产规则。
- [x] A. 提示词目录、统一来源、候选草稿与编辑界面。
  - 验收：现有发布版本不被覆盖；每个步骤标明来源、变量和实际规则。
- [x] B. 原文案锁定与视觉规划开关。
  - 依赖：A。验收：关闭零视觉模型调用，开启只规划视觉；任一原文字变化在生图前拦截。
- [x] C. 审核、修复和图像附加规则统一。
  - 依赖：A、B。验收：首稿、审核、重试引用同一快照，不再有隐藏人称或文风覆盖。
- [x] D. 仅管理员可见的完整请求追踪与可阅读界面（作业中心模型调用链路）。
  - 依赖：A。验收：管理员可查实际业务prompt、外层instructions、schema、数据来源与完整脱敏请求，截断明确标记，技术约束只读；普通用户和审核员无页面入口，直接读取 Web/中心接口被拒绝，普通任务响应不夹带内部请求或提示词快照。
  - 实现与验证：[管理员实际模型请求明细](../docs/model-request-visibility.md)。真实客户端内部未返回的子请求不伪称可见；旧记录不回填当前配置。
- [x] E. 检索、知识库和独立/离线/模拟入口覆盖。
  - 依赖：A、D。验收：所有目录项目已接入或明确显示兼容来源，中心与本地无静默混用。
- [x] F. 回归与小样本真实验收。
  - 依赖：B、C、D、E。先fake请求断言、中心兼容测试、类型/构建/浏览器，再做有界真实验证；保留历史快照恢复能力。

每批的细分文件、验收与风险见方案。生产图片裁切缺陷另行排期，不作为提示词编辑功能已修复的效果。

验收：主工程927/927、中心146/146；类型检查、生产构建和mock冒烟通过。隔离Chrome验证版本、预览、开关和历史；真实单页生图发现额外文字并经一次模型编辑、OCR复审通过。大快照、恢复缺失、人称误判、权限边界等回归已固化。详见 [实施与验收](../docs/prompt-governance-acceptance.md)。

---

# Codex 迁移任务（2026-09-05）

- [x] 读取当前接口、调用入口、并发/恢复机制及官方文档，完成迁移计划。
- [x] 1. 配置与工厂：CODEX 默认、OPENCLAW 回切、独立 Dots/DeepSeek 兼容测试。
- [x] 2. 进程协议：stdin、JSONL、退出/失败检测、超时取消、假进程测试。
- [x] 3. 模态：文本/审核/视觉/搜索/生图/改图接口及模拟原生工具产物的真实文件校验。
- [x] 4. 并发恢复：同机跨进程限制、限流冷却、认证/额度暂停与 Worker 接线。
- [x] 5. 全入口切换、CODEX 溯源、旧数据库迁移与界面文案。
- [x] 6. 全量测试、类型检查、构建、Mock 冒烟与预检/回切文档。
- [ ] 真实账户生成与持续并发验收（需实际调用，不能由 Mock 测试替代）。

验收记录（2026-09-05）：

- 主项目 `node --test "tests/*.test.mjs"`：702/702；中心服务：66/66；全部使用 fake/mock，不消耗模型额度。
- `npm run typecheck` 与最终 `npm run build` 通过；`npm run smoke` 完成，产物仍明确标为 `mock_only`，不具备发布资格。
- `npm run agent:check`：codex-cli 0.152.1，ChatGPT 登录，运行槽位 0、无本地暂停；不代表额度或生图能力已验证。
- 浏览器：隔离本机测试库完成 CODEX→OPENCLAW→CODEX 保存及刷新；模拟中心服务完成 CODEX 保存且保留 DEEPSEEK；两处控制台无 error/warn。临时浏览器页已关闭，3003/3004/4313 测试端口已停止。
- 回归中补齐原有中心图片恢复契约的 3 个失败测试：保留原执行 ID 链、限定原节点领取、缺失快照禁止默默重画。
- 安全/接口审查：参数数组、无 shell、业务密钥环境隔离、PNG 来源约束、超时结果未知不自动重放、失败上报可选 `autoRetry` 和中心健康能力门槛。
- `npm audit --omit=dev --audit-level=high`：0 高危/严重，2 项既有中危（ExcelJS 间接 uuid）；本次未增加依赖，未强制降级 ExcelJS。延后由独立依赖升级处理，复查日期 2026-09-12。
- 迁移/回切/真实验收说明：`docs/codex-exec-migration.md`。没有修改真实账号凭据、启动真实生成、部署远端服务或创建生产定时器。
...
- [x] P2a：规划思考等级、失败输出留存与局部修复；两条生产入口使用相同流程。
- [x] P2b：规划业务结构化输出约束；假 CLI 验证 schema、原有文本接口兼容。
- [x] P3：充分证据一次搜索结束；补搜及失败保留边界，复用任务快照。
...
- [x] 核对视图状态、角色入口、代理与中心任务查询、分页兼容及自动刷新逻辑。
- [x] 明确基础范围：管理员跨账号、跨执行机、全部九种任务状态及无创建者历史记录。
- [x] A1：新增管理员全部作业视图、菜单和页面权限；覆盖 IMAGE_FAILED、CANCELLED，并限制已废弃行操作。
- [x] A1 验收：三种角色来源均可见；普通用户保持个人任务隔离；审核员无法访问新管理员页面。
- [x] A2：接入状态筛选及完整服务端分页；旧中心不静默降级为前 200 条；自动刷新失败保留数据并提示。
- [x] A2 验收：250 条以上混合状态分页无截断，筛选总数正确，刷新失败和恢复提示正确。
- [x] A3：补视图、权限、分页与故障行为回归；更新工作台导航文档。
- [x] A3 验证：相关根目录及中心测试、类型检查、构建和模拟数据浏览器核验，全程不消耗模型额度。
- [x] 实施前检查点：用户追加确认角色筛选并授权实施；中心与 Web 已修改，未部署远程中心。

## 2026-09-06 作业列表排版与具体作业员筛选

- [x] 复现文本拆行拥挤、操作列移出可视区域，确认既有精确账号筛选和完整人员接口。
- [x] 增加作业员姓名/账号选择与组合查询；新增请求参数回归先失败后通过。
- [x] 合并相关列表列、限制长文本、提供完整内容详情入口和固定操作列。
- [x] 完成长文本/同名人员/组合分页/失败恢复浏览器验证及相关测试、类型和构建检查。

## 2026-09-06 列宽、分页与操作响应优化

- [x] 缩窄 Query 列并验证宽屏列比例。
- [x] 首页/尾页、指定页码、20/50/100 条分页、清空筛选。
- [x] 并行读取、终止浏览器过期请求、暂停隐藏页轮询、保留加载期布局。
- [x] 完成分页边界回归、受控延迟测试与真实浏览器核验。
