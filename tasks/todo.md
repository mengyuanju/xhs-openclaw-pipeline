...
- [ ] 增加知识条目、不可变版本、可选素材和任务引用表。
- [ ] 支持分页、状态流转、已发布配方检索和任务版本锁定。
- Verify：先写失败的 SQLite 集成测试，再实现并运行定向测试。
- Dependencies：Task 13。

## Task 15: 图片分析与双保留模式

- [ ] 使用 Sharp 校验图片，并通过 OpenClaw `infer model run --file` 提炼结构化配方。
- [ ] `PROMPT_ONLY` 不落盘；授权 `IMAGE_AND_PROMPT` 保存规范化 PNG。
- Verify：Fake runner、伪 MIME、超大图片、授权组合和临时文件清理测试。
- Dependencies：Task 14。

## Task 16: 视觉知识后台

- [ ] 增加认证 REST API、分页列表、创建/发布/归档和图片预览。
- [ ] 增加“视觉知识库”导航、上传分析表单和配方列表。
...
- [ ] 任务首次生成时匹配并锁定已发布配方版本。
- [ ] 主图组合全局规则、视觉配方和任务内容；授权图片加入参考图路径。
...
- [ ] README 记录模块边界、保留模式、真实分析成本和后续范围。
