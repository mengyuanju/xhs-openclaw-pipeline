...
- [ ] 1. 配置与工厂：CODEX 默认、OPENCLAW 回切、独立 Dots/DeepSeek 兼容测试。
- [ ] 2. 进程协议：stdin、JSONL、退出/失败检测、超时取消、假进程测试。
- [ ] 3. 模态：文本/审核/视觉/搜索/生图/改图及真实文件来源验证。
- [ ] 4. 并发恢复：同机跨进程限制、限流冷却、认证/额度暂停与 Worker 接线。
- [ ] 5. 全入口切换、CODEX 溯源、旧数据库迁移与界面文案。
- [ ] 6. 全量测试、类型检查、构建、Mock 冒烟与预检/回切文档。
