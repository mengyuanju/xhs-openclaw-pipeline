...
test('AI text worker targets one image, retries visual validation failures, and never uses a deterministic overlay',async()=>{
  ...
  const service={claim:async()=>({id:randomUUID(),task_id:1,target_page:1,operation:'TEXT',config}),context:async()=>({source:{id:1},refs:[],settings:{aiDisclosureEnabled:true,aiDisclosureText:'AI生成'},revision:{content:{imagePlan:[{headline:'真实参考'}]}},run:{result:{images:[{}]}}}),readAsset:async()=>source,heartbeat:async()=>true,
  ...
  const validateImage=async()=>generationCalls===0?visionPass():generationCalls===1
  ...
  try{const result=await processImageEdit({service,storageRoot:dir,workerId:'fake',agentClient,validateImage});assert.equal(result.status,'PREVIEW_READY');assert.equal(failed,false);assert.equal(prompts.length,2);assert.match(prompts[0],/AI_TEXT_EDIT/u);assert.match(prompts[0],/AI_DISCLOSURE/u);assert.match(prompts[1],/未通过文字验收/u);assert.match(inputs[0][0],/source\.png$/u);assert.match(inputs[1][0],/result\.png$/u);assert.equal(completed.validation.generationAttempts,2);assert.equal(completed.validation.model,'fake-text-edit');assert.equal(completed.validation.text.engine,'existing-vision-alignment');assert.equal(completed.validation.text.targetOccurrences,1);assert.equal(completed.validation.text.placement.passed,true);}finally{await rm(dir,{recursive:true,force:true});}
});
