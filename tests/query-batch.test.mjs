...
test('batch queries split only newlines and preserve Chinese/English commas in input order', () => {
...
test('duplicate queries are identified after trimming without silently dropping input', () => {
  const result = parseQueryBatch('收纳\n穿搭\n 收纳 ');
...
test('each query is limited to 500 Unicode characters rather than the whole textarea', () => {
  const valid = parseQueryBatch('好'.repeat(500) + '\n' + '🖼'.repeat(500));
