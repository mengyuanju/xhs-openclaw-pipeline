...
function validPost() {
  return {
    ...
    title: '桌面整理先做减法',
    body: '我先说结论：低成本整理卧室桌面，重点不是先添置容器，而是让物品位置跟着使用频率走。面对收完又乱的桌面，我会先看哪些动作每天重复，再决定清空、分区和复位顺序。\n\n先把桌面上的物品全部移开，保留真正需要每天使用的东西。不要急着购买收纳盒，先观察纸笔、充电器和文件最常出现的位置。把高频用品放在伸手能够拿到的区域，低频用品移入抽屉，并给每一类物品设定固定位置。完成分区后，只留下当天会使用的物品。每天睡前检查垃圾是否离桌、物品是否归位、第二天用品是否已经准备好。如果某个区域总是再次变乱，说明位置并不符合动作习惯，应调整位置而不是增加新的容器。连续执行几天后，再判断是否需要补充收纳工具。选择工具时按照物品尺寸和使用频率决定，避免为了填满空间而购买。最后拍照记录整理后的基准状态，之后按照这个状态完成一分钟复位。\n\n还要留意桌面的操作空间。容器数量越多，未必越容易保持整齐；拿取路径被挡住时，物品反而更容易临时堆放。先用已有纸盒或托盘试位置，确认动作顺手后再决定是否购买。\n\n归纳起来，低成本桌面整理要先减少无关物品，再按频率固定位置，最后靠每日复位维持。只要位置符合动作习惯，新增收纳工具就不再是第一选择。',
    tags: ['#桌面整理', '#租房生活', '#收纳方法'],
    imagePlan: [
