...
test('workbench list state safely round-trips filters and sorting through the URL', () => {
  const parsed = parseWorkbenchListState({
    ...
    createdByUserId: 'alice', createdByRole: 'USER', state: 'IMAGE_FAILED', attention: 'FAILED', taskId: '99',
  ...
  assert.equal(workbenchListSearch(parsed, { includeAdminFilters: true }).toString(),
    'page=3&pageSize=50&query=%2342&sort=createdAt%3Aasc&deduplicateQuery=1&state=IMAGE_FAILED&createdByUserId=alice&createdByRole=USER&attention=FAILED&taskId=99');
});
